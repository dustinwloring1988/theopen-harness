/**
 * Commander adapter for the `toh` command line.
 *
 ...
 * `@buckeyestudio/toh-cmdline`). Launcher flags therefore come first: the first
 * token this parser does not recognize starts the inner arguments, so
 * `toh --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `toh --profile web -h` prints the web app's help, not this one's.
 *
 * `web` is a hardcoded alias for `--profile web`; `plugin` manages a profile's
 * plugin dependencies by forwarding to pnpm.
 * @module @buckeyestudio/toh/args
 */
...
const HELP_EXAMPLES = `
Examples:
  toh --profile web                          boot the web profile (same as: toh web)
  toh --profile headless "run the tests"     answer one task, print the result, and exit
  toh --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
 ...
 */
function resolveBoot(program: Command, profile: string, options: BootOptions, args: string[]): TohInvocation {
 ...
 * @returns the resolved invocation.
 */
export function parseDshArgs(argv: readonly string[], version: string): TohInvocation {
  let resolved: TohInvocation | undefined
  ...
  program
    ...
    .description('toh: boot a TheOpen Harness profile — an ordered stack of plugin-bundle patch layers under your own overrides.')
    .addHelpText('after', HELP_EXAMPLES)
    .exitOverride()
    // The launcher's flags come first and end at the first token it does not
    // know; everything from there on belongs to the booted app, including
    // its -h. `toh -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: toh --profile <name> --help)')
    .option('--profile <name>', 'the profile under $TOH_HOME/profiles to boot')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    .option('--dump-config', 'print the composed profile tree and exit')
    .option('--dump-default-config', 'print the profile tree without its user layer or --patch overlays and exit')
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      // With the app owning -h, the launcher's own help is what a bare
      // `toh -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: toh web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('toh: no invocation resolved')
  return resolved
}
