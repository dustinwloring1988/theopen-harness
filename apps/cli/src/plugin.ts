/**
 * `toh plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `toh.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `toh.bundle` joins the layer stack; a
 ...
 * gained its `toh.bundle` declaration in a newer version.
 * @module @buckeyestudio/toh/plugin
 */
 ...
 * @returns true when the package manifest declares `toh.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.toh?.bundle?.patch !== undefined
}

/**
 * Reconcile `toh.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `toh.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.toh?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no toh.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (toh-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.toh = { ...after.toh, profile: { ...after.toh?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `toh` was invoked from.
...
/**
 * Run one `toh plugin` invocation: init if needed, forward to pnpm, reconcile.
