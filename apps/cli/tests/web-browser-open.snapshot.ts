/** Assembled keyless snapshot for the default `toh web` browser handoff. */
...
const openingMessage = 'toh web: opening the default browser; pass --no-open to disable'
...
if (process.env.TOH_EXAMPLE_MODE === 'lib' && !builtArtifactsExist) {
  throw new Error('toh web browser-open snapshot requires built CLI and Web artifacts in lib mode')
}
...
describe.skipIf(!builtArtifactsExist)('toh web browser-open assembled snapshot', () => {
  it('hands the reachable page to the default browser after the shipped tree settles', async () => {
    const root = mkdtempSync(join(tmpdir(), 'toh-web-browser-open-snapshot-'))
    tempRoots.push(root)
    ...
    const readyUrl = /toh web: (http:\/\/[^\s]+)/u.exec(result.stdout)?.[1]
    const openLine = result.stdout.split('\n').find(line => line.startsWith('toh browser-open: '))
    const opening = result.stdout.includes(openingMessage)
    if (readyUrl === undefined || openLine === undefined || !opening) {
      throw new Error(`toh web browser-open evidence missing\nstdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    const opened = JSON.parse(openLine.slice('toh browser-open: '.length)) as BrowserOpenRecord
    ...
    expect({
      ...
      tohHomePresent: opened.tohHomePresent,
      stderr: result.stderr,
    }).toMatchInlineSnapshot(`
      {
        "apiKeyPresent": false,
        "bootManifest": true,
        "tohHomePresent": false,
        "exitCode": 0,
  ...
  it('prints the launcher reason and manual URL after the Web app is ready', async () => {
    const root = mkdtempSync(join(tmpdir(), 'toh-web-browser-open-failure-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
      ...
      env: {
        ...
        DEEPSEEK_API_KEY: 'keyless-browser-open-no-call',
        TOH_AGENTS_HOME: join(root, '.agents'),
        TOH_BROWSER_OPEN_TEST_EXIT_ON_FAILURE: '1',
        TOH_HOME: join(root, '.toh'),
        TOH_TELEMETRY_DISABLED: '1',
        NODE_NO_WARNINGS: '1',
        SSH_CONNECTION: '',
    ...
    expect({
      diagnostic,
      exitCode: result.exitCode,
      opened: result.stdout.includes('toh browser-open: '),
  ...
  it('prints the host URL without launching a browser in a VS Code Remote SSH session', async () => {
    const root = mkdtempSync(join(tmpdir(), 'toh-web-browser-open-ssh-snapshot-'))
    tempRoots.push(root)
    const result = await execa(process.execPath, [
      '--import', openerHook,
      builtBin,
  ...
  it('rejects a project browser command before starting the Web app', async () => {
    const root = mkdtempSync(join(tmpdir(), 'toh-web-browser-open-env-snapshot-'))
    tempRoots.push(root)
    ...
    const diagnostic = result.stderr.split(/\r?\n/u)
      .find(line => line.startsWith('Error: toh: '))
      ?.replace(/^Error: toh: .*[/\\]\.env/u, 'toh: {{root}}/.env')
    ...
    expect({
      ...
      opening: result.stdout.includes(openingMessage),
      opened: result.stdout.includes('toh browser-open: '),
      ready: result.stdout.includes('toh web: '),
    }).toMatchInlineSnapshot(`
      {
        "diagnostic": "toh: {{root}}/.env sets "BROWSER", which only the launching environment may set (it decides how this process starts, where its code and instructions load from, or how it reaches the network); export BROWSER instead of putting it in a .env file",
        "exitCode": 1,
        "opened": false,
        "opening": false,
        "ready": false,
