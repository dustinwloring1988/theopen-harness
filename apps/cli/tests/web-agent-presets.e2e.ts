...
import { boot, healProfilesModuleFallback, loadOverlayPatches, loadProfile } from '@buckeyestudio/toh-app-boot'
...
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@buckeyestudio/toh-agent-presets'
import { applyChildComposition, childSessionMeta } from '@buckeyestudio/toh-subagent'
import { CallId } from '@buckeyestudio/toh-llm'
import type {} from '@buckeyestudio/toh-compaction-basic'
...
/** The shipped Web surface: the toh-base and toh-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$TOH_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $TOH_HOME. Unpinned, this
    // file writes the developer's own `~/.toh/storages/` — and then reads it
    ...
    // developer's own `~/.toh/.preset` cannot change this test's outcome.
  ...
  // Product Bundles are installed into the Profile, not the toh app. Model
  ...
  if (profileBundles !== undefined) {
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      ...
      toh: { profile: { bundles: profileBundles } },
    }, null, 2) + '\n')
    const profile = loadProfile('toh-test', 'spec', INSTALL_ANCHOR, home, { userLayer: false })
    bundlePatches = profile.layers.flatMap(layer => layer.patches)
  }
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('toh-test', rootConfig, [...bundlePatches, ...overrides], (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'toh-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'toh-preset-skill-proj-'))
    await mkdir(join(proj, '.toh', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.toh', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient TOH home,
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['toh-badge'])
      ...
      expect(scoped).toContain('toh-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by toh')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('toh-badge')
...
describe('product Bundle and user-preset intersection', () => {
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    const root = await mkdtemp(join(tmpdir(), 'toh-product-presets-'))
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<tohHome>/.agent-presets` itself. `$TOH_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'toh-preset-derived-'))
    ...
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'toh-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'toh-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'toh-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
