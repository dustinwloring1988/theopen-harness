...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@buckeyestudio/toh-loader-smoke'

const tohBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"toh-test: never-dispose ready", b"toh-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'toh-headless-shutdown-'))
  try {
    const home = join(cwd, '.toh')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'toh-profile-headless',
      private: true,
      dependencies: {},
      toh: { profile: { bundles: ['@buckeyestudio/toh-base', '@buckeyestudio/toh-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        TOH_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`toh headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`toh headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('toh: observing at ')
    expect(output).toContain('toh-test: never-dispose ready')
    expect(output).toContain('toh-test: never-dispose started')
