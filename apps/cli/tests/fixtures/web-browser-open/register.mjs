...
const exitMarker = join(process.cwd(), `.toh-browser-open-${process.pid}`)
...
if (process.env.TOH_BROWSER_OPEN_TEST_EXIT_ON_READY === '1') {
  const originalLog = console.log
  console.log = (...args) => {
    originalLog(...args)
    if (typeof args[0] === 'string' && args[0].startsWith('toh web: ')) {
      setTimeout(() => process.exit(0), 250)
    }
  }
}

if (process.env.TOH_BROWSER_OPEN_TEST_EXIT_ON_FAILURE === '1') {
