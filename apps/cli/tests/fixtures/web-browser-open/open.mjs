...
export default async function open(url) {
  ...
  console.log(`toh browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__TOH_BOOT__'),
    apiKeyPresent: process.env.DEEPSEEK_API_KEY !== undefined,
    tohHomePresent: process.env.TOH_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.toh-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
