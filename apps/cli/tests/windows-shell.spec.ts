 ...
 * the REAL shipped bundle layers (toh-base + toh-web-app resolved from the
...
import { entryListSchema } from '@buckeyestudio/cordis-plugin-include'
import { evaluate } from '@buckeyestudio/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@buckeyestudio/toh-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL toh-base/toh-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'toh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@buckeyestudio/toh-base', '@buckeyestudio/toh-web-app'])
    const profile = loadProfile('toh', 'web', anchor, home)
    ...
    for (const name of ['@buckeyestudio/toh-pwsh-sandbox', '@buckeyestudio/toh-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'toh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@buckeyestudio/toh-base'])
    const profile = loadProfile('toh', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
