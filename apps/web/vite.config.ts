...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__TOH_BOOT__. '
  + 'From a repository checkout, run `pnpm toh web`; an installed package uses `toh web`. '
  + 'For client-plugin HMR, run `pnpm toh web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'TOH Local Build'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.TOH_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'toh-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>TOH Local Build</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'toh-reject-standalone-web-serve',
    config(_config, env) {
