...
the deliberate composition divergences from `toh web` — are documented in
...
## Do not import `@buckeyestudio/toh-client-*` here
...
`@buckeyestudio/toh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@buckeyestudio/toh-client-web` and the boot-manifest
type from `@buckeyestudio/toh-client-modules/client`: booting the real shell is what
...
`@buckeyestudio/toh-client-runtime/client` because `client/runtime` is reachable
