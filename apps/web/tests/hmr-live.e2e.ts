/** Published toh web + pnpm dev:web → browser HMR, with no page reload. */
...
import type { SubprocessHandle, SubprocessSpawnSpec } from '@buckeyestudio/toh-subprocess'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'toh-web-hmr-world-'))
  const sourcePath = join(REPO_ROOT, 'packages/client/ui-conversation/src/client/locales.ts')
  const binPath = join(REPO_ROOT, 'apps/cli/lib/bin.js')
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built toh bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        ...
        TOH_HOME: join(world, '.toh'),
      },
    ))
    const baseUrl = await waitForOutput(host, /toh web: (http:\/\/[^\s]+)/, 'built toh web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__tohHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __tohHmrPageIdentity?: string }).__tohHmrPageIdentity))
      .toBe(pageIdentity)
