...
// fixture under the same record discipline as every other: TOH_SNAPSHOT=record drives the turn
...
import type { ContentBlock, Message } from '@buckeyestudio/toh-llm'
import { deriveEventMessage, SessionId } from '@buckeyestudio/toh-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    process.env.TOH_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
