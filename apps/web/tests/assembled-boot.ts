...
import { bootInjections, orderByModuleGraph } from '@buckeyestudio/toh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@buckeyestudio/toh-client-modules/client'
import { AppWebEntry } from '@buckeyestudio/toh-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@buckeyestudio/toh-app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares toh.client without a ./client export`)
  }
  return resolve(dirname(packagePath), relative)
}

/** Derive the assembled browser graph from the same bundle patches and package declarations as `toh web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.toh?.client
...
interface FixtureWindow extends Window {
  __TOH_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function installAssembledBootEnv(): void {
  beforeEach(() => {
    ...
    document.title = 'TheOpen Harness'
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  win.__TOH_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  const [facadeRow] = bootInjections(win.__TOH_BOOT__)
  ...
  for (const id of ['@buckeyestudio/toh-client-modules', '@buckeyestudio/toh-client-runtime']) {
 ...
 * the snapshot gate's `TOH_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.TOH_SNAPSHOT === 'record' || process.env.TOH_SNAPSHOT === 'refresh'
