...
import type {} from '@buckeyestudio/toh-agent-presets'
...
it('isolates replay skill discovery from every ambient host root', async () => {
  const ambient = await mkdtemp(join(tmpdir(), 'toh-web-ambient-skills-'))
  const tohHome = join(ambient, 'toh-home')
  const agentsHome = join(ambient, 'agents-home')
  const bundled = join(ambient, 'bundled')
  await Promise.all([
    writeSkill(join(tohHome, 'skills'), 'ambient-toh'),
    writeSkill(join(agentsHome, 'skills'), 'ambient-agents'),
    writeSkill(bundled, 'ambient-bundled'),
  ])

  const originalDshHome = process.env.TOH_HOME
  const originalAgentsHome = process.env.TOH_AGENTS_HOME
  const originalBundled = process.env.TOH_BUNDLED_SKILL_DIR
  process.env.TOH_HOME = tohHome
  process.env.TOH_AGENTS_HOME = agentsHome
  ...
  try {
    ...
    try {
      ...
      expect(names).not.toContain('ambient-toh')
    ...
    try {
      ...
      if (originalDshHome === undefined) delete process.env.TOH_HOME
      else process.env.TOH_HOME = originalDshHome
      if (originalAgentsHome === undefined) delete process.env.TOH_AGENTS_HOME
      else process.env.TOH_AGENTS_HOME = originalAgentsHome
      if (originalBundled === undefined) delete process.env.TOH_BUNDLED_SKILL_DIR
      else process.env.TOH_BUNDLED_SKILL_DIR = originalBundled
