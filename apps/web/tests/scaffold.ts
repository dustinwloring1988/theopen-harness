...
// Boots the REAL web composition — the toh-base and toh-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $TOH_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts toh-llm-replay in providers mode), record (real adapter + key,
...
//
// Composition divergences from `toh web`, all deliberate, all via include
...
import Include, { type PatchOptions } from '@buckeyestudio/cordis-plugin-include'
...
import {
  ...
  loadOverlayPatches,
} from '@buckeyestudio/toh-app-boot'
import { tohHomePath } from '@buckeyestudio/toh-home-paths'
...
import type { ReplayHandle } from '@buckeyestudio/toh-llm-replay'
import { installLlmReplay, parseSessionLog } from '@buckeyestudio/toh-llm-replay'
import SessionStore, {
  packChunkRuns,
...
import type {} from '@buckeyestudio/toh-host-webserver'
...
// } from '@buckeyestudio/toh-client-ui-settings-models'
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'TheOpen Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 TheOpen Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 TOH 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $TOH_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.TOH_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`TOH_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the toh-base and toh-web-app bundle patches over the empty profile root. */
...
export interface WebScaffold {
  ...
  /** Isolated harness home the settings/credentials rows write ($TOH_HOME double). */
  harnessHome: string
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted toh-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const workspaceCwd = await realpath(await mkdtemp(join(tmpdir(), 'toh-web-e2e-ws-')))
  // Isolated harness home: the settings/credentials rows resolve $TOH_HOME
  // paths at load, and an in-process boot must NEVER touch the developer's
  // real ~/.toh document or credential file.
  const harnessHome = options.harnessHome ?? join(workspaceCwd, '.toh-home')
  ...
  // session is created. Without this a developer's real ~/.toh/skills silently
  // enters replay requests and goldens while CI sees none. `TOH_HOME` follows
  ...
  const skillRootEnvironment = {
    TOH_HOME: harnessHome,
    TOH_AGENTS_HOME: join(workspaceCwd, '.agents-home'),
    TOH_BUNDLED_SKILL_DIR: join(workspaceCwd, '.bundled-skills'),
  }
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'toh-web-e2e-sessions-'))
  } catch (error) {
  ...
  // (bundle patches in toh.profile.bundles order), applied over the SAME empty root (a
  ...
  const patches: PatchOptions[] = [
    ...
    // Only the shipped root: a developer's own `~/.toh/.agent-presets` must not be
    ...
    // storage-json's yml root is anchored to the real $TOH_HOME; pin the row
    // to an absolute temp root (removed with the workspace at close) so tests
    // never write the user's harness home.
    { id: 'storage-json', config: { root: join(workspaceCwd, '.toh-storages') } },
    // Skill discovery is model-visible input. Pin every host-level root inside
    // the owned temp world so ~/.toh, ~/.agents, and a bundled-root env setting
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        tohHome: join(workspaceCwd, '.toh-home'),
    ...
    // to the production OTLP endpoint (or whatever TOH_TELEMETRY_OTLP_URL
    ...
    // (apps/web IS @buckeyestudio/toh-web-frontend); native browser opening and the
    ...
    { id: 'settings', config: { tohHome: harnessHome } },
    { id: 'credentials', config: { tohHome: harnessHome } },
    ...
    ...options.cordisTools === true
      ? [{ insert: [
        { id: 'tool-cordis', name: '@buckeyestudio/toh-tool-cordis' },
      ] }]
      : [],
  ...
  try {
    ...
    // `@buckeyestudio/cordis-plugin-group` by name.
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run TOH_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
