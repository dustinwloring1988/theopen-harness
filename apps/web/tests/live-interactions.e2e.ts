...
// composition and wire. The model adapter is toh-llm-replay with override
...
import type { RetryPolicyConfig } from '@buckeyestudio/toh-llm'
import { deriveReplayScript, parseSessionLog } from '@buckeyestudio/toh-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'toh-web-e2e-sidecar-'))
