...
The public package family belongs to the `@buckeyestudio` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@buckeyestudio/node-addon-landlock-run
@buckeyestudio/node-addon-landlock-run-<platform>
```
