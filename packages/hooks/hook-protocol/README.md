# @buckeyestudio/toh-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@buckeyestudio/toh-hooks-claude-code`, `@buckeyestudio/toh-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`toh-hook-protocol`) | The bridge (`toh-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `toh-hooks-claude-code` and `toh-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
