...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@buckeyestudio/toh-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@buckeyestudio/toh-hook-protocol'
import type { RunHookOptions } from '@buckeyestudio/toh-hook-protocol'
 ...
 * executor (toh-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what toh-bash-local's resolve does for the fields runHook sets.
      return {
