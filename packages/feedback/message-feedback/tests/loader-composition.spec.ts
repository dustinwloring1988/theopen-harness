...
import { remoteMethods } from '@buckeyestudio/toh-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@buckeyestudio/toh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@buckeyestudio/toh-storage', Storage],
    ['@buckeyestudio/toh-storage-json', StorageJson],
    ['@buckeyestudio/toh-storage-domain', StorageDomain],
    ['@buckeyestudio/toh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'toh-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "- name: '@buckeyestudio/toh-storage'",
      "- name: '@buckeyestudio/toh-storage-json'",
      '  config:',
