...
import * as CommandFeedback from '@buckeyestudio/toh-command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'toh-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@buckeyestudio/toh-command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { TOH_HOME: root } })
