...
import type { SessionProjectionRegistry } from '@buckeyestudio/toh-session-projection'
import type { SessionProjectionCache } from '@buckeyestudio/toh-session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @buckeyestudio/toh-session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @buckeyestudio/toh-session)',
