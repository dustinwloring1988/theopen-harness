...
import type { SubagentListEntry } from '@buckeyestudio/toh-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'toh-tool-list-agents-'))
  roots.push(root)
...
describe('toh-tool-subagent-control/list-agents', () => {
