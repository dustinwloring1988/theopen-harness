...
describe('task admission and package contracts', () => {
  it('ships one independently installable provider-only Bundle patch', () => {
    ...
    expect(manifest.toh?.bundle?.patch).toBe('./cordis.patch.yml')
    ...
    expect(manifest.dependencies).not.toHaveProperty('@buckeyestudio/toh-subagent-codex')
    ...
    const parsed = yaml.load(readFileSync(resolve(root, manifest.toh!.bundle!.patch!), 'utf8'))
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    vi.spyOn(ctx.subprocess, 'spawn').mockImplementation((spec) => {
      ...
      return spec.env?.TOH_CLAUDE_INSTANCE === 'safe'
        ? safeChild.handle
        : bypassChild.handle
    })
    ...
    expect(queryOptions.map(options => ({
      instance: options.env?.TOH_CLAUDE_INSTANCE,
    ...
    expect(spawnSpecs.map(spec => ({
      instance: spec.env?.TOH_CLAUDE_INSTANCE,
  ...
  it('starts through the registered provider with its resolved config and diagnostics', async () => {
    ...
    await ctx.plugin(claudeCode, {
      ...
      env: {
        ...
        CLAUDE_CONFIG_DIR: '/private/tmp/toh-claude-code-unit-config',
        HOME: '/private/tmp/toh-claude-code-unit-home',
      },
      permissionMode: 'auto',
      disposeGraceMs: 29,
    })
...
describe('query options and result mapping', () => {
  it('builds the fixed unattended options over the scrubbed environment', async () => {
    ...
    vi.stubEnv('TOH_INTERNAL', 'must-not-leak')
    const child = fakeChild()
    ...
    expect(options.env).not.toHaveProperty('TOH_INTERNAL')
