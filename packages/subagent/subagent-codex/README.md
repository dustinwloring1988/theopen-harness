...
- id: subagent-codex-safe
  name: '@buckeyestudio/toh-subagent-codex'
  config:
...
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a TOH interaction channel or per-call allow policy.
