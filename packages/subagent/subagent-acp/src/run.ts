 ...
 * @module @buckeyestudio/toh-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@buckeyestudio/toh-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `TOH_*` entries are deployment-owned facts for the child harness
   * (e.g. `TOH_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and TOH_* names
