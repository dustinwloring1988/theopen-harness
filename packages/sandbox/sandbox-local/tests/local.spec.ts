...
import { LAUNCHER_FAILURE_EXIT } from '@buckeyestudio/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@buckeyestudio/toh-sandbox'
import type { SandboxPolicy } from '@buckeyestudio/toh-sandbox'
import {
  LocalSandboxProvider,
} from '@buckeyestudio/toh-sandbox-local'
import type { Config } from '@buckeyestudio/toh-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'toh-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'toh-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'toh-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @buckeyestudio/toh-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'toh-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'toh-fake-acl-entry-'))
