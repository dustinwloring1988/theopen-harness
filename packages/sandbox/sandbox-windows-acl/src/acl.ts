 ...
 * @module @buckeyestudio/toh-sandbox-windows-acl/acl
 */
 ...
 * GetTempPathW (never from runner argv or TOH_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'toh-acl-locks', `${digest}.lock`)
}
