 ...
 * remain session-free.
 *
 * @module @buckeyestudio/toh-sandbox-policy
 */
...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from '@buckeyestudio/toh-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current TOH file policy: read-only. Any available operation enforced by the TOH file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current TOH file policy: workspace-write. Any available operation enforced by the TOH file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current TOH file policy: danger-full-access. The TOH file sandbox does not restrict file modifications by available operations.'
