 ...
 * `toh-fs-observation-policy`, so it is not exercised here.
 */
...
import type { FsTarget } from '@buckeyestudio/toh-fs'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'toh-fs-'))
  ctx = new Context()
...
describe('resolve', () => {
  it('resolves a relative path against opts.cwd, not config.cwd', async () => {
    ...
    const other = await mkdtemp(join(tmpdir(), 'toh-fs-other-'))
    try {
