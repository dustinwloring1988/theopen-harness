...
import { CodeRuntime } from '@buckeyestudio/toh-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@buckeyestudio/toh-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@buckeyestudio/toh-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@buckeyestudio/toh-llm'
import SystemPrompt from '@buckeyestudio/toh-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@buckeyestudio/toh-tools'
import type { Config as ToolConfig } from '@buckeyestudio/toh-tools'
import LocalFileSystem from '@buckeyestudio/toh-fs-local'
import * as FsPolicy from '@buckeyestudio/toh-fs-observation-policy'
import LocalAttachmentStore from '@buckeyestudio/toh-attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@buckeyestudio/toh-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'toh-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'toh-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { tohHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { tohHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { tohHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
