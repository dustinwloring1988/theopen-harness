...
import ToolRuntime, { type ToolResult } from '@buckeyestudio/toh-tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@buckeyestudio/toh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'toh-tool-fs-session-cwd-'))
