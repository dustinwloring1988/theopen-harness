 ...
 * model-facing render consumes ({@link module:@buckeyestudio/toh-tool-fs-search/search-core}
 ...
 * final output budget (`toh-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @buckeyestudio/toh-tool-fs-search/presentation
 */
 ...
 * ({@link module:@buckeyestudio/toh-tool-fs-search/grep} `formatGrepMatches`), so
