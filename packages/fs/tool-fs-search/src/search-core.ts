 ...
 *
 * @module @buckeyestudio/toh-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@buckeyestudio/toh-output-retention'
import type { RetainedItems } from '@buckeyestudio/toh-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@buckeyestudio/toh-subprocess'
import type { SaveTextSpill, SpillRef } from '@buckeyestudio/toh-spill'
 ...
 * `@buckeyestudio/toh-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`toh-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@buckeyestudio/toh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@buckeyestudio/toh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@buckeyestudio/toh-tool-fs-search/presentation} `grepSearchMeta`)
