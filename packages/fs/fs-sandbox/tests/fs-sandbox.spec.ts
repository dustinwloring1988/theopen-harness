...
import { FsError, FsTargetKey } from '@buckeyestudio/toh-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.toh-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'toh-fssbx-tmp-')), 'temp.txt')
