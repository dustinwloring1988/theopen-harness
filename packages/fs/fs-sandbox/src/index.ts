 ...
 * `@buckeyestudio/toh-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@buckeyestudio/toh-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@buckeyestudio/toh-tool-fs`),
 * exactly as bash's does.
 *
 * @module @buckeyestudio/toh-fs-sandbox
 */

import { Context } from '@buckeyestudio/cordis'
import { LocalFileSystem } from '@buckeyestudio/toh-fs-local'
import type { Config as LocalConfig } from '@buckeyestudio/toh-fs-local'
import { FsError } from '@buckeyestudio/toh-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@buckeyestudio/toh-fs'
import { writableRoots } from '@buckeyestudio/toh-sandbox'
 ...
 * INSTEAD OF `toh-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `toh-tool-fs` resolves
