...
`initialize` is the runtime-readiness boundary: when the server is mounted by a Loader composition, it waits for the current plugin tree to settle before replying, so async sibling capabilities such as initial MCP tool discovery are visible to the first prompt. Hand-built contexts without Loader remain immediately usable. `initialize.serverInfo.name` is the wire-stable `theopen-harness-sdk-runtime`. An optional positive `initialize.maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants; invalid values reject initialization, while omission sends no SDK cap and allows the selected adapter or provider route default to apply. Repeated initialization replaces a previously server-mounted fallback adapter: the previous fiber disposes before the replacement mounts, switching to a different registered provider disposes the obsolete fallback without a replacement, concurrent calls queue in arrival order, and an initialize that overlaps or follows shutdown rejects like any other post-shutdown request. Shutdown completes only after every handshake accepted before it began settles, including disposal of the adapter such a handshake replaced. `session/prompt` queues one identified user message and immediately returns `{ messageId }`. The server streams every durable fact as `session.event` and every whole-agent lifecycle transition as `session.status`; it does not assign an assistant message or `turn/end` to that prompt. Independent requests may enqueue more work on the same session. Persistence roots and persona come from `cordis.yml`.
...
- **Automatic adapter mounting is DeepSeek-specific** — `initialize` can reuse any pre-registered model adapter, but its only fallback mounts `toh-llm-deepseek`.
