...
describe('toh-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'toh-jsonrpc-apply-init-'))
    ...
    try {
      ...
      expect(response).toEqual({
        jsonrpc: '2.0',
        id: 'init-1',
        result: { serverInfo: { name: 'theopen-harness-sdk-runtime', version: '0.0.1' } },
      })
  ...
  it('does not answer initialize until async sibling Loader entries settle', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'toh-jsonrpc-apply-readiness-'))
    vi.stubEnv('DEEPSEEK_API_KEY', 'test-key')
    ...
    try {
      ...
      expect(response).toMatchObject({
        id: 'init-delayed',
        result: { serverInfo: { name: 'theopen-harness-sdk-runtime' } },
      })
    } finally {
      release()
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'toh-jsonrpc-apply-prompt-'))
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'toh-jsonrpc-apply-flush-failure-'))
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'toh-jsonrpc-apply-dispose-'))
    const harness = await mountPlugin(storageDir)
    try {
      ...
      expect(error.error).toMatchObject({
        code: -32603,
        message: 'unknown TheOpen Harness SDK runtime method: nope/unknown',
      })
