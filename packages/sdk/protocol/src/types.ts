/**
 * Named wire types for the TheOpen Harness SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@buckeyestudio/toh-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `theopen-harness-sdk-runtime`.
 *
 * @module @buckeyestudio/toh-sdk-protocol/types
 */

import type { ContentBlock } from '@buckeyestudio/toh-llm'
import type { SessionEvent } from '@buckeyestudio/toh-session'
import type { SubagentStopReason } from '@buckeyestudio/toh-subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`theopen-harness-sdk-runtime`) and version. */
