...
An unterminated incoming frame larger than `maxFrameBytes` (`JsonRpcLineTransportOptions`, default `DEFAULT_MAX_FRAME_BYTES` = 16 MiB, measured in UTF-8 bytes across chunk boundaries) cannot complete within the protocol, so the transport drops its buffer, destroys the input stream, and rejects every pending request with `JsonRpcResponseError` code `-32700`; later input is never retained. The cap bounds one partial frame, not the backlog of complete lines awaiting dispatch.
