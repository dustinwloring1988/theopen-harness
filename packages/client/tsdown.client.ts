...
const GENERATED_REMOTE = /^@buckeyestudio\/toh-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.TOH_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.TOH_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.TOH_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly toh?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `toh.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `toh.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'toh.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `toh.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).toh?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @buckeyestudio value import is a build error — a
      ...
      name: 'toh-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@buckeyestudio/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s toh.client.external, an inline-safe wire layer, or a generated /remote contribution — `
...
const STATIC_LINKED_PLUGIN = 'toh-static-linked-external'
