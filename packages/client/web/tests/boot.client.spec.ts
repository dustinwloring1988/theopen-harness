...
import * as modulesClient from '@buckeyestudio/toh-client-modules/client'
import type {
  ClientBundleRegistration, ClientModuleCreateOptions, ClientModuleLoaderTarget, TohWindow,
  WebBootEntry,
...
describe('bootstrap failure rendering', () => {
  ...
  it('renders a malformed boot manifest', async () => {
    await expectBootFailure(() => {
      installFacade()
      delete win.__TOH_BOOT__
    }, 'window.__TOH_BOOT__ is missing or not an object')
  })
  ...
  it('renders a module-system construction failure', async () => {
    await expectBootFailure(() => {
      ...
      win.__TOH_BOOT__ = { rev: 'graph', entries: [duplicate, duplicate] }
...
describe('plugin activation', () => {
  it('allows a modules-dependent row to be created before the modules row', async () => {
    ...
    win.__TOH_BOOT__ = { rev: 'graph', entries }
