/**
 * Package-owned invariant companion for `@buckeyestudio/toh-client-modules`.
 * @module @buckeyestudio/toh-client-modules/invariant
 */
...
const PACKAGE_NAME = '@buckeyestudio/toh-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __TOH_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
