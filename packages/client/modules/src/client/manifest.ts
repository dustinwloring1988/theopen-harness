 ...
 * `__TOH_BOOT__` wire types, the boot-manifest parser, and the boundaries around
 * {@link ClientModuleSystem}. The package root is the host-side service that
 * composes the wire.
 */

import type {} from '@buckeyestudio/cordis'
...
/**
 * Validate an optional string-array field read from a `toh.client` declaration
...
export function parseBootManifest(wire: unknown): BootManifest {
  if (typeof wire !== 'object' || wire === null) {
    throw new Error('client-modules: window.__TOH_BOOT__ is missing or not an object')
  }
