...
import { renderIndexInjections, type WebServer, type WebRoute } from '@buckeyestudio/toh-host-webserver'
...
const RUNTIME_ID = '@buckeyestudio/toh-client-runtime'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { toh: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'toh-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { toh: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__TOH_BOOT__"] = ')
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} toh.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@buckeyestudio/cordis'] }),
      entry('other'),
