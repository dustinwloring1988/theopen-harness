...
} from '@buckeyestudio/toh-api-remotes/client'
// Value import from the inline-safe wire layer (not the connection plugin):
// plugin-to-plugin value imports are a bundle purity error.
import { SESSION_SEARCH_RESULT_LIMIT } from '@buckeyestudio/toh-host-apiproxy/api'
import type {
...
// toh-scope, keyed by Agent identity); re-exported here so existing
...
export class SessionRuntime implements ISessions {
  ...
  constructor(
    ...
    this.selection = createSnapshotStore<SessionSelection>(
      {},
      { persist: { name: 'toh.sessions.current' } })
