...
import AgentPresets, { COMPOSITION_FILE, SETTINGS_NAMESPACE } from '@buckeyestudio/toh-agent-presets'
...
async function harness(
  ...
  const home = await mkdtemp(join(tmpdir(), 'toh-preset-settings-'))
...
describe('the default preset as a user setting', () => {
  ...
  it('clears a user default it has just deleted', async () => {
    const root = await mkdtemp(join(tmpdir(), 'toh-preset-authored-'))
