 ...
 * @module @buckeyestudio/toh-agent-presets/mount
 */
...
import type { EntryTree } from '@buckeyestudio/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@buckeyestudio/toh-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@buckeyestudio/toh-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `toh-invariants` is a
