...
import { CallId, type StreamChunk } from '@buckeyestudio/toh-llm'
import SessionStore, { type SessionEvent } from '@buckeyestudio/toh-session'
import type { SessionPersistence } from '@buckeyestudio/toh-session-persistence'
import SessionPersistenceJsonl from '@buckeyestudio/toh-session-persistence-jsonl'
import SessionPersistenceSqlite from '@buckeyestudio/toh-session-persistence-sqlite'
...
describe('SQLite cross-backend differential behavior', () => {
  it('preserves ignorable logical events whose names match physical storage tags', async () => {
    ...
    const directory = await freshDirectory('toh-sqlite-storage-tag-collision-')
  ...
  it('matches JSONL/Zstandard for every packed kind, scalar fallback, suffix, partition, and reopen', async () => {
    ...
    for (const [partitionIndex, sizes] of [[events.length], [1], [2, 1, 5, 3]].entries()) {
      const directory = await freshDirectory(`toh-sqlite-matrix-${partitionIndex}-`)
  ...
  it('matches JSONL/Zstandard across randomized logical logs and append partitions', async () => {
    await fc.assert(fc.asyncProperty(randomWorkload, async ({ events, batchSizes }) => {
      const directory = await freshDirectory('toh-sqlite-property-')
      for (const name of ['jsonl-zstd', 'sqlite'] as const) {
