...
import SessionStore, { SessionId, type SessionEvent } from '@buckeyestudio/toh-session'
import SessionPersistenceSqlite, {
  DEFAULT_BUSY_TIMEOUT_MS,
  SCHEMA_VERSION,
} from '@buckeyestudio/toh-session-persistence-sqlite'
import {
...
async function freshDbPath(prefix = 'toh-sqlite-'): Promise<string> {
...
runCoordinatorContract('sqlite', async (): Promise<CoordinatorFixture> => {
  const directory = await mkdtemp(join(tmpdir(), 'toh-sqlite-coord-'))
...
describe('SessionPersistenceSqlite physical packing', () => {
  it('loads from cordis.yml and packs through the assembled service', async () => {
    const path = await freshDbPath('toh-sqlite-loader-')
    const configPath = join(path, '..', 'cordis.yml')
    await writeFile(configPath, [
      "- name: '@buckeyestudio/toh-session'",
      "- name: '@buckeyestudio/toh-session-persistence-sqlite'",
      '  config:',
    ...
    ctx.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@buckeyestudio/toh-session') return SessionStore
        if (specifier === '@buckeyestudio/toh-session-persistence-sqlite') {
  ...
  it.runIf(process.platform !== 'win32')('bounds paced-stream WAL extent without rewriting committed rows', async () => {
    ...
    const measured = await measureWriteTraffic(await freshDbPath('toh-sqlite-traffic-'), events)
  ...
  it('includes a packed predecessor when an overlapping scalar tail hides it', async () => {
    const path = await freshDbPath('toh-sqlite-overlap-')
  ...
  it('waits for a competing process within the configured busy timeout', async () => {
    const path = await freshDbPath('toh-sqlite-busy-')
  ...
  it('rejects an older SQLite physical schema', async () => {
    const path = await freshDbPath('toh-sqlite-old-schema-')
  ...
  it('rejects a stale physical append without replacing the winning tail', async () => {
    const path = await freshDbPath('toh-sqlite-stale-')
  ...
  it('rejects a stale repair without deleting a newer winning tail', async () => {
    const path = await freshDbPath('toh-sqlite-stale-repair-')
...
describe('SessionPersistenceSqlite schema ownership', () => {
  it('accepts every configured journal mode and SQLite memory mode result', async () => {
    ...
    for (const mode of ['wal', 'delete', 'truncate', 'persist'] as const) {
      ...
      const path = await freshDbPath(`toh-sqlite-journal-${mode}-`)
  ...
  it('retries a busy journal-mode transition within its retry budget', async () => {
    const path = await freshDbPath('toh-sqlite-journal-busy-')
    let attempts = 0
  ...
  it('does not retry journal failures outside the available busy budget', async () => {
    for (const { errcode, timeout } of [
      ...
      await expect(openDatabase(
        FailingDatabase,
        await freshDbPath(`toh-sqlite-journal-failure-${errcode}-`),
        'wal',
        timeout,
  ...
  it('starts no journal retry after its open-relative cutoff', async () => {
    ...
    try {
      await expect(openDatabase(
        BusyDatabase,
        await freshDbPath('toh-sqlite-journal-cutoff-'),
        'wal',
        100,
  ...
  it('paces repeated busy journal-mode attempts', async () => {
    ...
    await expect(openDatabase(
      BusyDatabase,
      await freshDbPath('toh-sqlite-journal-paced-'),
      'wal',
      50,
    )).rejects.toThrow('database is locked')
  ...
  it('rejects unversioned, incompatible, and foreign-application databases', async () => {
    const unversionedPath = await freshDbPath('toh-sqlite-unversioned-')
    ...
    const incompatiblePath = await freshDbPath('toh-sqlite-incompatible-')
    ...
    const foreignPath = await freshDbPath('toh-sqlite-foreign-')
  ...
  it('rejects changed columns and non-strict owned tables', async () => {
    const changedPath = await freshDbPath('toh-sqlite-columns-')
    ...
    const nonStrictPath = await freshDbPath('toh-sqlite-nonstrict-')
    ...
    const loosePath = await freshDbPath('toh-sqlite-loose-')
  ...
  it('rejects invalid durable metadata before exposing a session header', async () => {
    const path = await freshDbPath('toh-sqlite-metadata-')
    const store = new SqliteStore({ path, journalMode: 'wal', busyTimeoutMs: DEFAULT_BUSY_TIMEOUT_MS })
...
describe('SessionPersistenceSqlite edge behavior', () => {
  it('keeps a fresh database unopened until the first persistence operation', async () => {
    const path = await freshDbPath('toh-sqlite-lazy-')
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionPersistenceSqlite, { path })
  ...
  it('disposes after path validation without opening the database', async () => {
    const path = await freshDbPath('toh-sqlite-unused-')
    const ctx = new Context()
    await ctx.plugin(SessionStore)
    await ctx.plugin(SessionPersistenceSqlite, { path })
    await ctx.fiber.dispose()
    ...
    const untouchedPath = await freshDbPath('toh-sqlite-never-validated-')
  ...
  it('rejects omitted torn markers and stale closer positions', async () => {
    const path = await freshDbPath('toh-sqlite-repair-validation-')
    const store = new SqliteStore({ path, journalMode: 'wal', busyTimeoutMs: DEFAULT_BUSY_TIMEOUT_MS })
  ...
  it('rejects malformed physical tail rows before appending', async () => {
    const path = await freshDbPath('toh-sqlite-tail-')
    const store = new SqliteStore({ path, journalMode: 'wal', busyTimeoutMs: DEFAULT_BUSY_TIMEOUT_MS })
  ...
  it('rejects missing and empty store identities', async () => {
    for (const mode of ['missing', 'empty'] as const) {
      const path = await freshDbPath(`toh-sqlite-identity-${mode}-`)
  ...
  it('rejects invalid paths during service initialization', async () => {
    const path = await freshDbPath('toh-sqlite-invalid-path-')
    const ctx = new Context()
    await ctx.plugin(SessionStore)
  ...
  it('rejects non-files and symbolic links', async () => {
    const directoryPath = await freshDbPath('toh-sqlite-directory-')
    await mkdir(directoryPath)
    expect(errorMessage(await backendFailure(directoryPath)))
      .toMatch(/must be a regular file/)

    const linkPath = await freshDbPath('toh-sqlite-link-')
    ...
    const parentLinkPath = await freshDbPath('toh-sqlite-parent-link-')
  ...
  it.runIf(
    ...
    const permissivePath = await freshDbPath('toh-sqlite-permissive-')
    ...
    const writableParentPath = await freshDbPath('toh-sqlite-parent-')
  ...
  it('surfaces database creation failures after path validation', async () => {
    const path = await freshDbPath('toh-sqlite-create-failure-')
    const store = new SqliteStore({ path, journalMode: 'wal', busyTimeoutMs: DEFAULT_BUSY_TIMEOUT_MS })
