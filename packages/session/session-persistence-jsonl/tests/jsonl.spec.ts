import { MessageId, createUserMessage, createMessage } from '@buckeyestudio/toh-llm'
...
import type { Session, SessionEvent, SessionHeader } from '@buckeyestudio/toh-session'
...
async function freshRoot(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'toh-jsonl-'))
  dirs.push(dir)
  return dir
}
...
runCoordinatorContract('jsonl-none', async (): Promise<CoordinatorFixture> => {
  const dir = await mkdtemp(join(tmpdir(), 'toh-jsonl-coord-'))
  return {
    mount: async (ctx) => {
      const fiber = await ctx.plugin(JsonlSessionPersistence, { root: dir, compression: 'none' })
      return fiber
...
describe('JsonlSessionPersistence: format helpers', () => {
  ...
  it('projectKey normalizes project paths into bounded readable names', () => {
    expect(projectKey('/Users/qyj/work/theopen-harness')).toBe('--Users-qyj-work-theopen-harness--')
