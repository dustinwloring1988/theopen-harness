...
The JSONL durable session-persistence backend — a concrete `SessionPersistence` (the `toh-session-persistence` seam). Each session has one append-only logical JSONL log, stored as `.jsonl.zstd` by default or raw `.jsonl` when compression is disabled.
...
- **Startup orphaned-temp sweep.** Mounting the backend deletes every `*.tmp` file inside each session directory below the root whose mtime predates this process's start: publication never targets those names, so a stale temporary proves its writer died between the temp write and publication, while a newer one may belong to a live process sharing the root and waits for a later mount. Non-matching files and logs are untouched, and the sweep is best-effort — a successful sweep logs its count at debug, an enumeration failure logs its error at debug without rejecting the mount, and per-file failures are skipped for the next startup.
