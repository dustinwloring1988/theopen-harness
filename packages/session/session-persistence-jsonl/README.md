...
- **Startup orphaned-temp sweep.** Mounting the backend deletes every `*.tmp` file inside each session directory below the root: publication always targets the fixed log name, so a surviving temporary proves the writing process died between the temp write and publication. Non-matching files and logs are untouched, and the sweep is best-effort — enumeration or per-file failures log at debug and never reject the mount.
