...
import { getOrCreateAnonymousUserId } from '@buckeyestudio/toh-anonymous-user-id'
import Loader from '@buckeyestudio/cordis-plugin-loader'
import { recordFeedback } from '@buckeyestudio/toh-command-feedback'
...
// pin TOH_HOME to a temp dir so the suite never touches the ambient ~/.toh.
let tempHome: string
let previousDshHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'toh-otel-home-'))
  previousDshHome = process.env.TOH_HOME
  process.env.TOH_HOME = tempHome
})
afterAll(() => {
  if (previousDshHome === undefined) delete process.env.TOH_HOME
  else process.env.TOH_HOME = previousDshHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'theopen-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@buckeyestudio/toh-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@buckeyestudio/toh-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@buckeyestudio/toh-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('toh-session-telemetry-otel real-load-path guard', () => {
