...
npm scope: `@buckeyestudio/toh-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@buckeyestudio/toh-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`memory/`](memory/README.md) | Cross-session fact memory: provider-registry seam, storage-domain provider, and model-facing memory tools | Product — stable API |
...
| [`bundle/`](bundle/README.md) | Installable `toh --profile` patch layers | Product — stable API |
