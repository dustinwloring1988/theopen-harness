...
import {
  ...
  type CommandResult,
  type Sandbox,
} from '@buckeyestudio/toh-e2b'
import type E2BRuntime from '@buckeyestudio/toh-e2b'
import type { SubprocessSpawnSpec } from '@buckeyestudio/toh-subprocess'
import E2BSubprocessRuntime from '@buckeyestudio/toh-subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0TOH_STALE=old\0BROKEN\0=bad\0'
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.toh-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.toh-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$toh_e2b_env_bin" -i'))!
    expect(command).toContain('"$toh_e2b_setsid" --wait -- "$toh_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('TOH_MODE')
    ...
    expect(command).not.toContain('${!toh_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$toh_e2b_home" -a -d "$toh_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('toh_e2b_node="$(command -v node)"')
    expect(command).toContain('"$toh_e2b_env_bin" -i "$toh_e2b_node" -e')
    expect(command).toContain('"$toh_e2b_env_bin" -i -- "${toh_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$toh_e2b_env_bin" -i -- "${toh_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.toh-e2b/processes/one/pid',
      '/workspace/.toh-e2b/processes/one/exit-code',
      '/workspace/.toh-e2b/processes/one/environment',
      '/workspace/.toh-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.toh-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0TOH_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.toh-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('toh_e2b_tee='))!
    expect(command).toContain('"$toh_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$toh_e2b_tee" --output-error=warn-nopipe')
