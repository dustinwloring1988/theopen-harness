...
import { FsTargetKey, FsVersion } from '@buckeyestudio/toh-fs'
import E2BFileSystem from '@buckeyestudio/toh-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.toh-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['toh-version']).toBeDefined()
