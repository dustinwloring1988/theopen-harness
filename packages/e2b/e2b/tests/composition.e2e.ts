...
import { Inbox } from '@buckeyestudio/toh-agent'
import type { Agent } from '@buckeyestudio/toh-agent'
import { runLoaderSmoke } from '@buckeyestudio/toh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@buckeyestudio/toh-e2b'
import TerminalSessionService, { TerminalSessionId } from '@buckeyestudio/toh-terminal'
import { BashTerminalBackend } from '@buckeyestudio/toh-terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', TOH_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/toh-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'toh_leak=0',
          'for toh_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$toh_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$toh_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then toh_leak=1; fi',
          'done',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> TOH=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$TOH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> TOH=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'toh-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.toh-e2b/terminals/')
