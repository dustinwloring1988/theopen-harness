...
import type { ShellRunResult, CollectedOutput } from '@buckeyestudio/toh-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@buckeyestudio/toh-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@buckeyestudio/toh-sandbox'
...
import type { Config } from '@buckeyestudio/toh-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'toh-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'TOH_WRAP=1', 'bash', '-c', 'printf "%s" "$TOH_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$TOH_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'toh-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.TOH_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'toh-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'toh-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
