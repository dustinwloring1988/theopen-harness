 ...
 * trusted, per-execution `TOH_*` variables consumed by the model-facing shell
 * tools (`toh-tool-bash`, `toh-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @buckeyestudio/toh-shell-env
 */

import { Service, type Context } from '@buckeyestudio/cordis'
import z from '@buckeyestudio/schemastery'
import { TOH_ENV_PREFIX } from '@buckeyestudio/toh-shell'
import type { TohEnvironment, TohEnvironmentKey } from '@buckeyestudio/toh-shell'
import { TOH_HOME_ENV, resolveDshHome } from '@buckeyestudio/toh-home-paths'
import type { ToolExecution } from '@buckeyestudio/toh-tools'
import type {} from '@buckeyestudio/toh-session-persistence'
...
/** Model-visible metadata for one managed `TOH_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `TOH_*` keys this contributor may return. */
  variables: Readonly<Record<TohEnvironmentKey, BashEnvVariable>>
  /**
   ...
   */
  resolve(execution: ToolExecution): Readonly<Partial<Record<TohEnvironmentKey, string>>>
}
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `TOH_*` environment variable name. */
  key: TohEnvironmentKey
}

const TOH_SHELL_KEY = `${TOH_ENV_PREFIX}SHELL` as const
const TOH_SESSION_ID_KEY = `${TOH_ENV_PREFIX}SESSION_ID` as const
const TOH_SESSION_JSONL_KEY = `${TOH_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<TohEnvironmentKey>([
  TOH_HOME_ENV,
  TOH_SHELL_KEY,
  TOH_SESSION_ID_KEY,
])
...
/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `TOH_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `TOH_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<TohEnvironmentKey, string>()
  private readonly tohHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.tohHome = resolveDshHome(config.tohHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [TohEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(TOH_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(TOH_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): TohEnvironment {
    const values: Record<TohEnvironmentKey, string> = {
      [TOH_HOME_ENV]: this.tohHome,
      [TOH_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[TOH_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as TohEnvironmentKey
 ...
 * shell-agnostic persistence contributor (`TOH_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [TOH_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
