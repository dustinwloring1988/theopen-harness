 ...
 * managed `TOH_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@buckeyestudio/toh-system-prompt'
...
import type { ShellProcessRead } from '@buckeyestudio/toh-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, tohHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, tohHome === undefined ? {} : { tohHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, tohHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed TOH_* environment', async () => {
    const tohHome = mkdtempSync(join(tmpdir(), 'toh-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, tohHome)
    ...
    expect(request?.tohEnv).toEqual({
      TOH_HOME: tohHome,
      TOH_SHELL: '1',
      TOH_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const tohEnv = bash.requests[0]?.tohEnv
    expect(tohEnv).toBeDefined()
    expect(tohEnv?.['TOH_SHELL']).toBe('1')
    expect(tohEnv?.['TOH_HOME']).toEqual(expect.any(String))
    expect(tohEnv).not.toHaveProperty('TOH_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'toh-tool-pwsh-policy-'))
