 ...
 * `@buckeyestudio/toh-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `toh-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `TOH_*` environment through the
 ...
 * exit-status parse from `@buckeyestudio/toh-shell`.
 *
 * @module @buckeyestudio/toh-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@buckeyestudio/cordis'
import z from '@buckeyestudio/schemastery'
import { defineTool, TOOL_ABORTED } from '@buckeyestudio/toh-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@buckeyestudio/toh-tools'
...
import type { SandboxExecutionPolicy, SandboxMode } from '@buckeyestudio/toh-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@buckeyestudio/toh-sandbox'
import type { SandboxPolicyService } from '@buckeyestudio/toh-sandbox-policy'
import type { ShellRunResult } from '@buckeyestudio/toh-shell'
import { parseExitStatus } from '@buckeyestudio/toh-shell'
...
/* jscpd:ignore-start -- minimal mirror of toh-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:TOH_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror toh-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of toh-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of toh-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of toh-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors toh-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with toh-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors toh-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        tohEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @buckeyestudio/toh-jobs and @buckeyestudio/toh-tool-jobs')
        }
