 ...
 *
 * @module @buckeyestudio/toh-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors toh-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@buckeyestudio/cordis'
import z from '@buckeyestudio/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@buckeyestudio/toh-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@buckeyestudio/toh-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@buckeyestudio/toh-subprocess'
import { installSettingsSection } from '@buckeyestudio/toh-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@buckeyestudio/toh-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of toh-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `toh-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.tohEnv },
    }
  }
   ...
   * `toh-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@buckeyestudio/toh-pwsh-sandbox`.
