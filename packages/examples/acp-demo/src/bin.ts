 ...
 * `toh-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in toh-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @buckeyestudio/toh-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@buckeyestudio/toh-app-boot'

const NAME = 'toh-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   toh-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['TOH_SNAPSHOT']
