...
ACP automation server app: the default agent spine, client-created agents through [`@buckeyestudio/toh-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `@buckeyestudio/toh-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `@buckeyestudio/toh-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `@buckeyestudio/toh-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `@buckeyestudio/toh-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `@buckeyestudio/toh-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `persona` | — | Deployment persona template for `toh-system-prompt`. |
| `toolOrder` | lexicographic | Explicit model-facing tool order for `toh-system-prompt`. |
| `tools` | `{ mode: 'native' }` | Native, Code Mode, or combined model tool transport. |
| `tohHome` | `$TOH_HOME` or `~/.toh` | Harness home shared by bash and local skill discovery. |
...
`toh-acp-demo [--config path-to-cordis.yml]` (short form `-c`; default `./cordis.yml`) loads the gitignored `.env`, except in replay mode; `TOH_SNAPSHOT=replay` selects the sibling `cordis.snapshot.yml`; stdin EOF disposes the context and flushes sessions before exit. Loader's installed optional `node-addon-require-builtin` peer resolves bare plugin specifiers for the built bin under plain Node. Diagnostics use stderr because stdout is the ACP wire.

## Model Experience

Indirectly, through `toh-agent-spine-demo` and the leaf's model-facing plugins. ACP prompt text becomes the ordinary logged user message; protocol metadata and permission choices do not enter the model request.
