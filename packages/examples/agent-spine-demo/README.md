...
```
@buckeyestudio/cordis-plugin-timer  timer service (writes nothing to stdout)
@buckeyestudio/toh-llm              abstract LLM service + content-block vocabulary
@buckeyestudio/toh-session          event-sourced session log + store
@buckeyestudio/toh-session-title    log-backed title service + deterministic fallback
@buckeyestudio/toh-system-prompt    prompt-section + tool-schema assembly
@buckeyestudio/toh-tools            registry + guarded pre/around/post/final-result pipeline
@buckeyestudio/toh-skill            skill provider registry
@buckeyestudio/toh-skill-filesystem      local filesystem skill provider
@buckeyestudio/toh-agent            agent registry + initiator scope + agent/* events
@buckeyestudio/toh-goal             optional persisted same-session goal domain
@buckeyestudio/toh-tool-goal        optional model-facing goal controls
@buckeyestudio/toh-goal-round-driver     optional same-session goal-round driver
@buckeyestudio/toh-llm-retry        provider-routed request retry policy
@buckeyestudio/toh-jobs-local      generic background-job registry
@buckeyestudio/toh-invariants       configurable invariant registry service
@buckeyestudio/toh-session/invariant
@buckeyestudio/toh-agent/invariant
@buckeyestudio/toh-scope/invariant
@buckeyestudio/toh-agent-loop/invariant
                                  package-owned relational checks
@buckeyestudio/toh-tool-bash        the model-facing bash schema (unless toolBash=false)
@buckeyestudio/toh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@buckeyestudio/toh-tool-skill       session-prefix skill catalog + model-facing loader schema
@buckeyestudio/toh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@buckeyestudio/toh-agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (toh-system-prompt gets the forwarded `persona`)
```
...
```ts
import type { Config } from '@buckeyestudio/toh-agent-spine-demo'
// { agents?, maxParallelToolCalls?, includeHarnessIdentity?, includeRuntimeContext?, persona?, toolOrder?, tools?, tohHome?, sessionTitle?, skills?, workspaceContext, toolBash?, jobs?, toolJobs?, goals?, invariants? }
// workspaceContext requires { maxBytes } or false; the other owner schemas supply defaults.
```

The bundle forwards each field to the child that owns it. App packages supply any pre-created agents: headless and JSON-RPC compositions create `main`, while the ACP app creates agents on demand at `session/new`. `includeRuntimeContext: false` is forwarded to `toh-system-prompt` and suppresses all dynamic context snapshots for fresh sessions without disabling their policy services. Prompt, tool, title, skill, agent-instructions, invariant, goal, and task settings retain the schemas and defaults documented by their owning packages; `jobs.maxConcurrentJobsPerOwner` configures the local provider independently of the model-facing `toolJobs` controls. `pickSpineConfig()` copies only fields owned by this bundle, and conflicting `tohHome` values fail during composition.

For example, `{ invariants: { enabled: true, package_allowlist: ['^@buckeyestudio/toh-'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`toh-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
...
Indirectly, through `toh-system-prompt`, `toh-tool-skill`, `toh-tool-bash`, `toh-tools`, and `toh-llm-retry`, plus `toh-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
