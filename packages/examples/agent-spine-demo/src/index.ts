 ...
 * @module @buckeyestudio/toh-agent-spine-demo
 */

import type { Context } from '@buckeyestudio/cordis'
import Timer from '@buckeyestudio/cordis-plugin-timer'
...
import SessionTitleService, { type Config as SessionTitleConfig } from '@buckeyestudio/toh-session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@buckeyestudio/toh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@buckeyestudio/toh-tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@buckeyestudio/toh-skill'
import * as SkillFileSystem from '@buckeyestudio/toh-skill-filesystem'
import AgentRegistry from '@buckeyestudio/toh-agent'
import GoalService, { type Config as GoalDomainConfig } from '@buckeyestudio/toh-goal'
import * as goalSession from '@buckeyestudio/toh-goal-round-driver'
...
import InvariantRegistry, { type Config as InvariantConfig } from '@buckeyestudio/toh-invariants'
...
import * as bashEnv from '@buckeyestudio/toh-shell-env'
...
import * as toolJobs from '@buckeyestudio/toh-tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@buckeyestudio/toh-agent-loop'
import * as llmRetry from '@buckeyestudio/toh-llm-retry'
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'tohHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.tohHome !== undefined ? { tohHome: config.tohHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedDshHome = config.skills?.filesystem?.tohHome
  if (config.tohHome !== undefined && nestedDshHome !== undefined
    && resolveDshHome(config.tohHome) !== resolveDshHome(nestedDshHome)) {
    throw new Error('agent-spine-demo: tohHome and skills.filesystem.tohHome must resolve to the same directory')
  }
  const tohHome = resolveDshHome(config.tohHome ?? nestedDshHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { tohHome }))
  }
