...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@buckeyestudio/toh-tools'
import type { WebSearchResult, WebSearchSource } from '@buckeyestudio/toh-web'
import type {} from '@buckeyestudio/toh-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `toh-tool-fs`'s
