...
import type { WebSearchProvider, WebSearchResult } from '@buckeyestudio/toh-web'
...
} from '@buckeyestudio/toh-tool-web'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@buckeyestudio/toh-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by toh-tool-call-timeout-policy over exec.signal.
