/**
 * Integration: the real fetch backend (`toh-web-fetch-http`) + a real search provider
 * (`toh-web-search-exa`) + the real seam (`toh-web`) + the model tool (`toh-tool-web`) + the
 * tool-call timeout policy (`toh-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@buckeyestudio/toh-tools'
...
import * as TimeoutPolicy from '@buckeyestudio/toh-tool-call-timeout-policy'
...
beforeEach(async () => {
  ...
  // The loopback fixture is the trusted-composition opt-in for the provider's
  // private-network guard.
  await ctx.plugin(WebFetchLocal, { allowPrivateNetworks: true })
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  beforeEach(async () => {
    ...
    // The loopback fixture is the trusted-composition opt-in for the private-network guard.
    await tctx.plugin(WebFetchLocal, { timeoutMs: 30_000, allowPrivateNetworks: true })
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by toh-tool-call-timeout-policy,
