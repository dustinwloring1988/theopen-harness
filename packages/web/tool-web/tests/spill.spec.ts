 ...
 * (`toh-spill-local` backend + `toh-spill-policy`), exercised through
...
import LocalSpillStore from '@buckeyestudio/toh-spill-local'
import * as SpillPolicy from '@buckeyestudio/toh-spill-policy'
...
beforeEach(async () => {
  ...
  spillRoot = mkdtempSync(join(tmpdir(), 'toh-spill-web-'))
  ...
  await ctx.plugin(WebFetchLocal, { maxBodyChars: 500_000, allowPrivateNetworks: true })
