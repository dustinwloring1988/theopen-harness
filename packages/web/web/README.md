...
| Package | Role |
|---|---|
| `@buckeyestudio/toh-web` (this) | Service Definition: the service, provider registries, selection policy, request/result vocabulary, the `WebError` taxonomy |
| `@buckeyestudio/toh-web-search-exa` | Search provider: Exa |
| `@buckeyestudio/toh-web-search-perplexity` | Search provider: Perplexity |
| `@buckeyestudio/toh-web-fetch-http` | Fetch provider: anonymous public HTTP(S) |
| `@buckeyestudio/toh-tool-web` | Consumer: the model-facing `web_search` / `web_fetch` tool schemas over `ctx.web` |
...
Providers register **capabilities**, not tools. `toh-tool-web` is the only owner of model-facing names, descriptions, prompt guidance, JSON schemas, and presentation.
...
Indirectly, through `toh-tool-web`, which retains bounded normalized provider data or the exact configured-provider, unavailable-provider, no-provider, multiple-provider, and `Error: <message>` failures while this registry contributes no prompt or schema itself.
