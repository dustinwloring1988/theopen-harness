...
import { ExaSearchProvider, EXA_PROVIDER_ID } from '@buckeyestudio/toh-web-search-exa'
import * as exaPlugin from '@buckeyestudio/toh-web-search-exa'
...
describe('ExaSearchProvider error handling', () => {
  ...
  it('parses a success reply at exactly the byte ceiling', async () => {
    const body = new TextEncoder().encode(JSON.stringify({ results: [{ url: 'https://a.test', highlights: ['hi'] }] }).padEnd(MAX_RESPONSE_BYTES, ' '))
    expect(body.byteLength).toBe(MAX_RESPONSE_BYTES)
    vi.stubGlobal('fetch', vi.fn(async () => streamResponse([body])))
    await expect(new ExaSearchProvider(options).search({ query: 'q' }))
      .resolves.toMatchObject({ sources: [{ url: 'https://a.test', snippet: 'hi' }] })
  })
  ...
  it('refuses a success reply that streams past the byte ceiling', async () => {
    ...
    await expect(new ExaSearchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({
        ...
        message: `Exa answered with more than ${MAX_RESPONSE_BYTES} bytes`,
      }))
  })
