 ...
 * @module @buckeyestudio/toh-web-search-exa/provider
 */
...
export class ExaSearchProvider implements WebSearchProvider {
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    if (!response.ok) {
      ...
      try {
        const parsed = JSON.parse(await readBounded(response)) as ExaError
    ...
    try {
      const payload = JSON.parse(await readBounded(response)) as ExaSearchResponse
...
async function readBounded(response: Response): Promise<string> {
  const oversized = (): WebError =>
    new WebError(`Exa answered with more than ${MAX_RESPONSE_BYTES} bytes`, 'WEB_PROVIDER_ERROR')
