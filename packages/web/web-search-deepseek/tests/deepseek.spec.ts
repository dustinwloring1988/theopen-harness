...
describe('DeepSeekSearchProvider error handling', () => {
  ...
  it('parses a success reply at exactly the byte ceiling', async () => {
    const body = new TextEncoder().encode(JSON.stringify(searchResponse()).padEnd(MAX_RESPONSE_BYTES, ' '))
  ...
  it('refuses a success reply that streams past the byte ceiling', async () => {
    ...
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({
        code: 'WEB_PROVIDER_ERROR',
        message: `DeepSeek answered with more than ${MAX_RESPONSE_BYTES} bytes`,
      }))
  })
