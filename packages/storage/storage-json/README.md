...
- Overlapping writes serialize their publications on a per-unit chain: the newest acknowledged state lands last, so a resolved write is never discarded by an older overlapping one. Ordering across separately awaited calls belongs to the caller (the domain layer's write chain); each single call is atomic and durable once resolved.
