...
SQLite backend for the [storage hub](../storage/README.md): registers as backend `sqlite`, serving the `kv` facet over one `node:sqlite` database file (or `:memory:`). Design and trade-offs: [domain KV storage Agent Note](../../../.agents/notes/proposed/architecture/2026-07-24-domain-kv-storage-and-workspace.md). The shipped composition keeps the JSON default; [`examples/web-storage-sqlite`](../../../examples/web-storage-sqlite/README.md) is the opt-in Web overlay that routes the `workspace` domain here.
