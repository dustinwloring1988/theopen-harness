...
When the store mounts, it sweeps crash-orphaned staging residue once: entries of the private `tmp/` directory whose mtime predates this process's start, plus every `*.tmp` file beside a request-image cache object (publication never targets those names). Stored objects and cached variants are untouched. The sweep is best-effort; enumeration or per-file failures log at debug and never block the mount.
