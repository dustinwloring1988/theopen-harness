...
存储挂载时会进行一次崩溃残留暂存文件清扫：私有 `tmp/` 目录中的条目以及请求版本缓存对象旁的 `*.tmp` 文件，各自仅在其自身修改时间早于本进程启动时才被收集。已发布工件从不指向这些名称，而本进程在挂载时尚未创建任何文件，因此较新的候选可能属于共享根目录的存活进程，将幸存至下次挂载。已存储对象和缓存变体不受影响。清扫是尽力而为的；成功的清扫以 debug 记录数量，枚举失败以 debug 记录错误且绝不会阻塞挂载，单文件失败则跳过留待下次启动。
