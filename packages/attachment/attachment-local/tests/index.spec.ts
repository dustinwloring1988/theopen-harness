...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const tohHome = await mkdtemp(join(tmpdir(), 'toh-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { tohHome })
      ...
      await rm(tohHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const tohHome = await mkdtemp(join(tmpdir(), 'toh-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { tohHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const tohHome = await mkdtemp(join(tmpdir(), 'toh-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { tohHome })
  ...
  it('prepares every batch member before any write', async () => {
    const tohHome = await mkdtemp(join(tmpdir(), 'toh-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { tohHome, normalizedImageMaxBytes: 1 })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const tohHome = await mkdtemp(join(tmpdir(), 'toh-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { tohHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { tohHome, maxImageBytes: 1 })
