...
| Package | Role |
|---|---|
| `@buckeyestudio/toh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@buckeyestudio/toh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@buckeyestudio/toh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
...
Providers register **capabilities**, not tools. `toh-tool-lsp` is the only owner of the model-facing name, description, prompt guidance, schema, and presentation.
...
Indirectly, through `toh-tool-lsp`, which owns the model-facing `lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.
