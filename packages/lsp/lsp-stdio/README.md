...
| `env` | `{}` | Extra env merged on top of the credential-scrubbed ambient env (vars matching `KEY`/`PASSWORD`/`SECRET`/`TOKEN` are not forwarded); an explicit `TOH_*` entry merges after the seam's scrub of ambient ones. |
...
Indirectly, through `toh-tool-lsp`, which surfaces this provider's normalized results; this host contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `toh-tool-lsp` owns request-prefix changes.
