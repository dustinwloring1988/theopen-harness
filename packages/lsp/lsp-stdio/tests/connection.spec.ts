...
describe('LspConnection', () => {
  ...
  it('forwards explicit TOH_* env entries to the child', async () => {
    // A configured TOH_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'TOH_LSP_TEST_FACT', TOH_LSP_TEST_FACT: 'managed' })
