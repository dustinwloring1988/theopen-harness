...
/**
 * Keyless built-artifact smoke: plain Node imports `@buckeyestudio/toh-lsp` and
 * `@buckeyestudio/toh-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@buckeyestudio/toh-lsp')
      const LspLocal = await import('@buckeyestudio/toh-lsp-stdio')
      const { default: LocalFileSystem } = await import('@buckeyestudio/toh-fs-local')
