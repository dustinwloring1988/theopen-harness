...
import { CompactionId, compactCheckpointSource } from '@buckeyestudio/toh-compaction'
...
import SessionQueryEngine from '@buckeyestudio/toh-session-query'
...
} from '@buckeyestudio/toh-session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a toh-session: URI?')).toEqual({
      text: 'what is a toh-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see toh-session:%%%')).toEqual({
      text: 'see toh-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see toh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](toh-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `toh-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('toh-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
...
describe('session reference discovery and preparation', () => {
  ...
  it('does not prepare a rejected pre-step and rejects malformed direct mentions', async () => {
    ...
    const malformed = createUserMessage({
      content: [{ type: 'text', text: '@[bad](toh-session:not-canonical)' }],
      source: { kind: 'user' },
    })
