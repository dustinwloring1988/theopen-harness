...
import ToolRegistry, { defineContentToolFixture } from '@buckeyestudio/toh-tools'
import { FILE_REFERENCE_PROMPT } from '@buckeyestudio/toh-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'toh-file-reference-service-'))
  roots.push(root)
