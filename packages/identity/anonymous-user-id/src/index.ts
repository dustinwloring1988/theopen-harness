 ...
 * harness home resolved by {@link resolveDshHome} (`$TOH_HOME` > `~/.toh`),
 ...
 * machine: every process sharing one `$TOH_HOME` reports the same id, and
 ...
 *
 * @module @buckeyestudio/toh-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `TOH_HOME`; defaults to `process.env`. */
