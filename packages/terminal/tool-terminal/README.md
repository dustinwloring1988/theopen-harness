...
The six generated schemas are listed in the [`toh-tool-terminal` catalog section](../../../docs/tool-catalog.md#buckeyestudiotoh-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@buckeyestudio/toh-jobs` and its model-facing controller.
