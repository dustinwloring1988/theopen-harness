...
import AgentRegistry, { Inbox, type Agent } from '@buckeyestudio/toh-agent'
...
import SandboxPolicyService, { setSandboxMode } from '@buckeyestudio/toh-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@buckeyestudio/toh-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@buckeyestudio/toh-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@buckeyestudio/toh-terminal-bash'
import { ENCODING_PREAMBLE } from '@buckeyestudio/toh-pwsh-local'
import * as ptyLocal from '@buckeyestudio/toh-terminal-bash'
import type { ResolvedConfig } from '@buckeyestudio/toh-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@buckeyestudio/toh-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'toh> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'toh> \'',
        TOH_SHELL: '1', TOH_SESSION_ID: 'agent', TOH_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo toh> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo toh> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', TOH_SHELL: '1', TOH_SESSION_ID: 'agent', TOH_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'toh> ' : 'PowerShell 7.6.4\n',
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'toh> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
