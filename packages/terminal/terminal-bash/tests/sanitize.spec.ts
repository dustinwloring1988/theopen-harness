...
import { normalizeTerminalText, TerminalSanitizer } from '@buckeyestudio/toh-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07dsh> ')).toEqual({ text: 'toh> ', prompt: true, promptTail: 'toh> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('toh> ')).toEqual({ text: 'toh> ', prompt: false, promptTail: 'toh> ' })
  })
