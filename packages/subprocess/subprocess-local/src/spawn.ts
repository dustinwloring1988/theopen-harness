 ...
 * @module toh-subprocess-local/spawn
 */
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'toh-subprocess-'))
...
export class OutputCollector {
  ...
  private spillAll(chunk: Buffer): void {
    ...
    if (this.spillFd === undefined) {
      ...
      this.spillFile = join(
        this.spillDir,
        `toh-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
      )
