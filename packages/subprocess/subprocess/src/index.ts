 ...
 * @module @buckeyestudio/toh-subprocess
 */

import { Context, Service } from '@buckeyestudio/cordis'
import { TOH_ENV_PREFIX } from './types.ts'
 ...
 * `TOH_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `TOH_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `toh_*` entry
 * would otherwise survive and read back as `$env:TOH_*` in the child;
 * deliberate lowercase `toh_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(TOH_ENV_PREFIX)) env[key] = value
  }
  return env
}
