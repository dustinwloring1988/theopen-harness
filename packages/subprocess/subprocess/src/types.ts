 ...
 * @module toh-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for TheOpen Harness-managed child environment facts. */
export const TOH_ENV_PREFIX = 'TOH_' as const

/** One environment key inside the managed {@link TOH_ENV_PREFIX} namespace. */
export type TohEnvironmentKey = `${typeof TOH_ENV_PREFIX}${string}`

/** Trusted TheOpen Harness variables for one child-process execution. */
export type TohEnvironment = Readonly<Record<TohEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@buckeyestudio/toh-terminal` without a cross-seam dependency;
