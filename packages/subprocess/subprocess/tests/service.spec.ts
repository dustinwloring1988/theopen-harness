...
import { scrubbedParentEnv, SubprocessRuntime } from '@buckeyestudio/toh-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and TOH_ names (case-insensitively) but keeps PATH', () => {
    process.env.TOH_SCRUB_PROBE = 'stale'
    process.env.toh_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.TOH_SCRUB_PROBE).toBeUndefined()
      expect(env.toh_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.TOH_SCRUB_PROBE
      delete process.env.toh_scrub_probe_lower
