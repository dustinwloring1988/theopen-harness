...
import * as GoalSessionInvariant from '@buckeyestudio/toh-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@buckeyestudio/toh-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@buckeyestudio/toh-invariants'
import SessionStore, { SessionId, type Session } from '@buckeyestudio/toh-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@buckeyestudio/toh-goal-round-driver',
    }))
  })
