import { createAssistantMessage, createUserMessage } from '@buckeyestudio/toh-llm'
import { afterEach, describe, expect, it, vi } from 'vitest'
import { Context, type Fiber } from '@buckeyestudio/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@buckeyestudio/toh-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@buckeyestudio/toh-session'
import SessionPersistence, { SessionPersistenceRevision } from '@buckeyestudio/toh-session-persistence'
import type { SessionPersistenceSnapshot } from '@buckeyestudio/toh-session-persistence'
import SqliteSessionPersistence from '@buckeyestudio/toh-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@buckeyestudio/toh-session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'toh-session-search-'))
