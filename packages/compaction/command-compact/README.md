# @buckeyestudio/toh-command-compact
...
- id: command-compact
  name: '@buckeyestudio/toh-command-compact'
```

The shipped `toh` base mounts it beside `compaction-basic`, and the Web client provides the command adapter. Automation surfaces that compose no command adapter keep automatic compaction only.
