/**
 * End-to-end tests for toh-mcp-client. Exercises the REAL MCP protocol against:
...
import { publicToolName } from '@buckeyestudio/toh-mcp-client/src/tools.ts'
...
async function mountImageRegistry(tohHome: string): Promise<Context> {
  const ctx = await mountRegistry()
  await ctx.plugin(LocalAttachmentStore, { tohHome })
