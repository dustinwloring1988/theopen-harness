...
Prompt 同步与工具共用同一套监督机制：每个重连世代都会在工具交换之后重新执行 `prompts/list`，`notifications/prompts/list_changed` 会在存活世代上触发重新同步，重连预算耗尽时目录随工具注销一起清空。分页仅在 `nextCursor` 缺失时结束；服务器重复返回同一个游标（包括每页都回显空字符串）会使抓取失败并包含在同步内。短暂中断期间，最后正常的候选保持可见，而加载会稳妥地失败。
