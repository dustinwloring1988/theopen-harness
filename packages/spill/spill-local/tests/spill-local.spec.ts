...
import type { SaveTextSpill } from '@buckeyestudio/toh-spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@buckeyestudio/toh-spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'toh-spill-test-'))
})
afterEach(() => {
