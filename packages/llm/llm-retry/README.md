...
The executor has no policy config. Multi-provider adapters such as `toh-llm-pi-ai` place `retryPolicy` inside each provider profile, avoiding a second provider-name list.
