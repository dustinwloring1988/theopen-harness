...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from '@buckeyestudio/toh-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from '@buckeyestudio/toh-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'toh-llm-retry-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@buckeyestudio/toh-llm-retry', retry],
    ['@buckeyestudio/toh-agent-loop', AgentLoop],
  ])
