...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'toh-llm-composition-'))
  vi.stubEnv('TOH_HOME', root)
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@buckeyestudio/toh-settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@buckeyestudio/toh-credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@buckeyestudio/toh-settings-file', FileSettingsProvider],
    ['@buckeyestudio/toh-credentials-local', LocalCredentialProvider],
    ['@buckeyestudio/toh-llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-theopen-harness-user-id']).toBe(getOrCreateAnonymousUserId())
