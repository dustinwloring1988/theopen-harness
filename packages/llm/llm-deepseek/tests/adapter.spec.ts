...
import { AttachmentId, ImageVariantId } from '@buckeyestudio/toh-attachment'
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@buckeyestudio/toh-attachment'
import { createLaunchEnvironmentSnapshot } from '@buckeyestudio/toh-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@buckeyestudio/toh-anonymous-user-id'
import { SessionId } from '@buckeyestudio/toh-session'
import * as LlmDeepSeek from '@buckeyestudio/toh-llm-deepseek'
import { DeepSeekAdapter, resolveAdapterOptions } from '@buckeyestudio/toh-llm-deepseek'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'toh-llm-deepseek-'))
  vi.stubEnv('TOH_HOME', testHome)
})
...
describe('DeepSeekAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    expect(server.headers[0]?.['x-theopen-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-theopen-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-theopen-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `toh-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    expect(server.headers[1]?.['x-theopen-harness-compact']).toBe('1')
  })
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-theopen-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-theopen-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-theopen-harness-compact']).toBe('1')
  })
...
describe('plugin registration and config', () => {
  ...
  it('takes DEEPSEEK_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.toh/.env', values: { DEEPSEEK_BASE_URL: 'https://user.example' } },
    ])
