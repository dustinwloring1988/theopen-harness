...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId, createMessage } from '@buckeyestudio/toh-llm'
import type { Message, ToolSchema } from '@buckeyestudio/toh-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@buckeyestudio/toh-attachment'
import type {
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'toh-e2e-user-id-'))
  vi.stubEnv('TOH_HOME', identityHome)
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('llm-deepseek e2e (real API)', () => {
  ...
  it('serves a real request with the key held only by a credentials-local document', async () => {
    const key = process.env.DEEPSEEK_API_KEY
    if (key === undefined) throw new Error('e2e ran without DEEPSEEK_API_KEY')
    const dir = await mkdtemp(join(tmpdir(), 'toh-e2e-credentials-'))
    try {
