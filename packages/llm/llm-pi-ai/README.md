...
Every request carries the shared attribution header from toh-llm's `attributionHeaders()`, merged through pi-ai's `headers` stream option. Provider-specific app-attribution headers are not synthesized. See [toh-llm § App attribution](../llm/README.md#app-attribution-attributionts).
