...
/** A context whose credential records live in a throwaway `$TOH_HOME`. */
async function stored(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'toh-pi-auth-'))
  dirs.push(dir)
...
describe('pi-ai ambient auth context', () => {
  ...
  it('answers about the host filesystem, expanding a leading ~', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'toh-pi-home-'))
    dirs.push(dir)
