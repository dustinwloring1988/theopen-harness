...
import type { AttachmentStore, ImageAttachmentRef, ImageRequestPolicy, RequestImageAttachment } from '@buckeyestudio/toh-attachment'
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@buckeyestudio/toh-llm'
import type { ContentBlock, StreamChunk } from '@buckeyestudio/toh-llm'
...
describe('toPiContext', () => {
  ...
  it('degrades the flat pre-envelope replay state a legacy session log carries', () => {
    ...
    expect(context.messages[0]).toMatchObject({ role: 'assistant', api: 'toh-foreign' })
