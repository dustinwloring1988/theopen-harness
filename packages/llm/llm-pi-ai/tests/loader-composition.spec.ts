...
import LlmRuntime, { createMessage, createUserMessage } from '@buckeyestudio/toh-llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'toh-pi-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@buckeyestudio/toh-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
