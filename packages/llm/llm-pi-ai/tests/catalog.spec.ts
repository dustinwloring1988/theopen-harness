...
import LlmRuntime, { createUserMessage, ReasoningEffortId } from '@buckeyestudio/toh-llm'
...
/** A throwaway $TOH_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'toh-pi-catalog-'))
  homes.push(dir)
