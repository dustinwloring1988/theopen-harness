...
import type { CredentialKey, CredentialProvider, CredentialRecord } from '@buckeyestudio/toh-credentials'
import { launchEnvironmentOf } from '@buckeyestudio/toh-launch-environment'
import { LlmError } from '@buckeyestudio/toh-llm'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      ...
      + ' credential a sign-in produces; mount one (toh-credentials-local) to sign in',
      'NO_CREDENTIAL_STORE',
    )
  }
  return credentials
