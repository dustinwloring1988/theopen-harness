...
import { resolveRetryPolicy, RetryPolicySchema } from '@buckeyestudio/toh-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@buckeyestudio/toh-llm'
import {
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with toh-llm-retry',
    )
  }
}
