/**
 * File-backed credentials provider over `$TOH_HOME/.credentials.yaml`, layered
 ...
 * > $TOH_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… toh`, a CI
 ...
 * @module @buckeyestudio/toh-credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@buckeyestudio/toh-atomic-write'
import { canonicalizeWatchPath, resolveDshHome } from '@buckeyestudio/toh-home-paths'
import { launchEnvironmentOf } from '@buckeyestudio/toh-launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@buckeyestudio/toh-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
  ...
  CredentialRef,
  ResolvedCredential,
} from '@buckeyestudio/toh-credentials'
import type { LaunchEnvironmentEntry } from '@buckeyestudio/toh-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveDshHome(config.tohHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `toh-atomic-write`, this is a
...
/** File-backed credentials provider (`$TOH_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start toh from instead',
      )
    }
  }
