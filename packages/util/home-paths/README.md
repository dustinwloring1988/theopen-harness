...
`resolveDshHome()` resolves the single-root TheOpen Harness home. Precedence, highest first: an explicit configured path, `$TOH_HOME`, then `~/.toh`. The harness keeps all user data under one root.

`tohHomePath(...segments)` joins child segments onto that resolved home with Node's platform path rules. With no segments it returns the home itself.

`tohHomeDisplay()` names an active root symbolically for user-facing paths: `~/.toh` for the default home, `$TOH_HOME` for any configured home. It never leaks an absolute machine path.

`TOH_HOME_DIR_NAME` owns the default user-data directory name: `.toh`.

`defaultDshHome()` returns the default TheOpen Harness home by joining the operating-system home directory with `.toh`, using Node's platform path rules.
