...
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@buckeyestudio/toh-llm'
import { SettingsProvider, settingsNamespace } from '@buckeyestudio/toh-settings'
import type { SettingsNamespace } from '@buckeyestudio/toh-settings'
import { CredentialProvider } from '@buckeyestudio/toh-credentials'
import type {
  CredentialInfo,
  CredentialKey,
  CredentialRecord,
...
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@buckeyestudio/toh-agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('toh-settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('toh-credentials-local')
  })
