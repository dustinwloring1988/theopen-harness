...
import type { SessionHeader, SessionId } from '@buckeyestudio/toh-session'
import type { SessionLineageNode } from '@buckeyestudio/toh-session-query'
import type { SessionRawArtifact } from '@buckeyestudio/toh-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@buckeyestudio/toh-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('toh-session-session-root.zip')
