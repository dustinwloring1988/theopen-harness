...
import { TypertLookupFailure } from '@buckeyestudio/toh-typert-protocol'
import TypertRegistry from '@buckeyestudio/toh-typert-registry'
import { createUserMessage, MessageId } from '@buckeyestudio/toh-llm'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'toh-cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'toh-cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@buckeyestudio/toh-session').Session
