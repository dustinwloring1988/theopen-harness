...
import type { AttachmentStore, ImageAttachmentRef } from '@buckeyestudio/toh-attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@buckeyestudio/toh-session-query'
import type { SessionId, SessionStore } from '@buckeyestudio/toh-session'
import type { SessionPersistence, SessionRawArtifact } from '@buckeyestudio/toh-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `toh-session-${safeSessionIdSegment(sessionId)}.zip`
}
