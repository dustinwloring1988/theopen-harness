/**
 * Package-owned invariant companion for `@buckeyestudio/toh-host-webserver`.
 * @module @buckeyestudio/toh-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@buckeyestudio/toh-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__toh_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__toh_invariant_upgrade_probe__', handler: () => {} }
