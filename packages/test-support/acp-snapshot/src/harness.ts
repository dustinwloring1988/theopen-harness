 ...
 *
 * @module @buckeyestudio/toh-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `toh-llm-replay` via `$TOH_SNAPSHOT_CHILD_FILES` so each child
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      TOH_SNAPSHOT: opts.mode,
      TOH_SNAPSHOT_FILE: opts.fixtureFile,
      TOH_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      TOH_SNAPSHOT_SPILL_ROOT: spillRoot,
      TOH_HOME: join(cwd, '.toh'),
      TOH_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { TOH_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { TOH_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in toh-llm-replay's loadSessionScripts
