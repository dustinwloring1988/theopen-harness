 ...
 * @module @buckeyestudio/toh-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@buckeyestudio/cordis'
import type {} from '@buckeyestudio/toh-compaction'
import { decodeStorageRecord, type SessionEvent } from '@buckeyestudio/toh-session'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@buckeyestudio/toh-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.TOH_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $TOH_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.TOH_SNAPSHOT_OVERRIDE
  const childEnv = process.env.TOH_SNAPSHOT_CHILD_FILES
