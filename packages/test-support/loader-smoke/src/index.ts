 ...
 * zero-build dev path, resolving `@buckeyestudio/toh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @buckeyestudio/toh-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'TOH_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.TOH_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated TOH homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { TOH_HOME: join(cwd, '.toh'), TOH_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
