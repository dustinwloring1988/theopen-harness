 ...
 * process-local registry lives in `@buckeyestudio/toh-jobs-local`.
 * @module @buckeyestudio/toh-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@buckeyestudio/toh-jobs is the abstract job registry seam; load an implementation such as @buckeyestudio/toh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
