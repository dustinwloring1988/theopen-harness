 ...
 * leaf-level diff.
 * @module @buckeyestudio/toh-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@buckeyestudio/toh-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveDshHome(config.tohHome), 'settings.yaml'))
