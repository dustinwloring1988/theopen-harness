/**
 * @buckeyestudio/toh-headless — one-shot direct Agent driver. The bundle patch
 * rides over toh-base without Host, HTTP, or browser plugins; this runner
 ...
 *
 * @module @buckeyestudio/toh-headless
 */
...
import type { ModelSelectionRef } from '@buckeyestudio/toh-agent'
import type {} from '@buckeyestudio/toh-agent-default-model'
...
import type {} from '@buckeyestudio/toh-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`toh: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`toh: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
