 ...
 * @module @buckeyestudio/toh-headless/startup
 */
...
function headlessCommand(): Command {
  return new Command()
    ...
    .addHelpText('after', `
Examples:
  toh --profile headless "run the tests"     answer one task and exit
`)
}
...
export function apply(ctx: Context): void {
  ...
  program.action(() => {
    ...
    if (task.trim() === '') program.error('error: a task is required, for example: toh --profile headless "run the tests"')
