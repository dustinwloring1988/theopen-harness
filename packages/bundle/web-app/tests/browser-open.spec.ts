...
import WebServer from '@buckeyestudio/toh-host-webserver'
...
afterEach(async () => {
  ...
  vi.unstubAllEnvs()
  Reflect.deleteProperty(globalThis, '__tohWebAppApply')
  Reflect.deleteProperty(globalThis, '__tohWebServer')
})

describe('web app browser startup', () => {
  it('opens the canonical URL only after the complete page is reachable', async () => {
    const root = mkdtempSync(join(tmpdir(), 'toh-web-browser-open-'))
    tempRoots.push(root)
    ...
    writeFileSync(webserverModule, 'export default globalThis.__tohWebServer\n')
    writeFileSync(webAppModule, [
      "export const name = 'fixture-web-app'",
      "export const inject = ['webServer']",
      'export const apply = (ctx, config) => globalThis.__tohWebAppApply(ctx, config)',
      '',
    ].join('\n'))
    ...
    const globals = globalThis as unknown as {
      __tohWebAppApply: typeof apply
      __tohWebServer: typeof WebServer
    }
    globals.__tohWebAppApply = apply
    globals.__tohWebServer = WebServer
