...
import TypertRegistry from '@buckeyestudio/toh-typert-registry'
import type { TypertContribution } from '@buckeyestudio/toh-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@buckeyestudio/toh-tool-cordis/src/api-catalog.ts'
...
describe('model-driven toh-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@buckeyestudio/toh-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('toh-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@buckeyestudio/toh-tools')
    ...
    const record = ctx.typert.getPackage('@buckeyestudio/toh-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@buckeyestudio/toh-tools', 'host')).toBeUndefined()
  })
})
