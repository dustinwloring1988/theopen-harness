...
} from '@buckeyestudio/toh-typert-registry'
import type {
...
function toolsContribution(schema: z.ZodType = z.object({ name: z.string() })): TypertContribution {
  return {
    package: '@buckeyestudio/toh-tools',
    face: 'host',
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@buckeyestudio/toh-tools', 'ToolInput')).toBe('@buckeyestudio/toh-tools#ToolInput')
    expect(typertPackageKey('@buckeyestudio/toh-tools', 'host')).toBe('@buckeyestudio/toh-tools#host')
    expect(ctx.typert.get('@buckeyestudio/toh-tools#ToolInput')).toMatchObject({
      package: '@buckeyestudio/toh-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@buckeyestudio/toh-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@buckeyestudio/toh-tools', 'host')).toMatchObject({
      key: '@buckeyestudio/toh-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@buckeyestudio/toh-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@buckeyestudio/toh-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@buckeyestudio/toh-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@buckeyestudio/toh-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@buckeyestudio/toh-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@buckeyestudio/toh-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@buckeyestudio/toh-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@buckeyestudio/toh-tools#ToolInput')).not.toBe(projected)
  })
