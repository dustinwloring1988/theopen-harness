 ...
 *
 * @module @buckeyestudio/toh-skill-filesystem
 */
...
import type Schema from '@buckeyestudio/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@buckeyestudio/toh-fs'
...
import {
  ...
  type SkillSource,
} from '@buckeyestudio/toh-skill'

const PROJECT_TOH_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_TOH_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.TOH_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.TOH_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.toh/skills'), source: 'project-toh', rank: PROJECT_TOH_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.tohHome, 'skills'), source: 'user-toh', rank: USER_TOH_RANK, skipSystem: true },
