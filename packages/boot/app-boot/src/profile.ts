 ...
 * `toh --profile` launcher family.
 *
 * A profile is a directory under `$TOH_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `toh.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"toh": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `toh.profile.bundles` order over
 ...
 * first from the toh installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$TOH_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @buckeyestudio/toh-app-boot/profile
 */
...
import type { EntryOptions } from '@buckeyestudio/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@buckeyestudio/cordis-plugin-include'
import { resolveDshHome } from '@buckeyestudio/toh-home-paths'
...
/** The bundle half of the `toh` manifest section: what a bundle package exports. */
export interface TohBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `toh` manifest section: what a profile directory composes. */
export interface TohProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `toh`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface TohManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: TohBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: TohProfileManifest
}
...
export interface ProfileManifest {
  ...
  toh?: TohManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `toh.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `toh.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`toh --profile <name>`).
...
export function resolveProfileDir(name: string, home: string = resolveDshHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    ...
    || name === 'node_modules') {
    throw new Error(`toh: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@buckeyestudio/toh-base', '@buckeyestudio/toh-web-app'],
  headless: ['@buckeyestudio/toh-base', '@buckeyestudio/toh-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@buckeyestudio/toh-base', '@buckeyestudio/toh-web-app', '@buckeyestudio/toh-headless'],
}

/** The bundle list a `toh plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@buckeyestudio/toh-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this toh profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `toh.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `toh-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      toh: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`toh: ${link} exists and is not a symlink; remove it so toh can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$TOH_HOME/profiles/node_modules`: one
 * symlink per package in the toh app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`toh-compaction`, `toh-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the toh app's package.json.
 * @param home - the Harness home; defaults to {@link resolveDshHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveDshHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (toh-subprocess,
    // toh-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.toh?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    toh: {
      ...manifest.toh,
      profile: { ...manifest.toh?.profile, bundles: [...current] },
    },
  }
 ...
 * `@buckeyestudio/toh-base` (and every other in-box bundle) always comes from
 * the same installation as the running toh, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `toh.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the toh app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the toh installation or ${profileDir}; `
    + `run 'toh plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `toh.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `toh.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the toh app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveDshHome}.
...
export function loadProfile(
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'toh plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the toh section entirely.
  const bundles = manifest.toh?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.toh?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no toh.bundle in its package.json`)
    }
