...
const NAME = 'toh-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'toh-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'TOH_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['TOH_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['TOH_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'TOH_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['TOH_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['TOH_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a harness switch', 'TOH_PERMISSION_MODE=danger-full-access\n'],
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@buckeyestudio', 'toh-system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@buckeyestudio', 'toh-system-prompt')
  ...
  it('exposes tohHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const tohHome = join(dir, 'home')
    vi.stubEnv('TOH_HOME', tohHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js tohHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(tohHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by TheOpen Harness.')
