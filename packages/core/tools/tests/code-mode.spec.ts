...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@buckeyestudio/toh-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@buckeyestudio/toh-tools'
import type { Agent } from '@buckeyestudio/toh-agent'
import { Session, SessionId } from '@buckeyestudio/toh-session'
import type { JsonValue, SessionEventMap } from '@buckeyestudio/toh-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@buckeyestudio/toh-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `toh-agent-tool-presentation` produces.
