 ...
 * execution pipeline.
 * @module @buckeyestudio/toh-tools
 */

import { Context, Service } from '@buckeyestudio/cordis'
import z from '@buckeyestudio/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@buckeyestudio/toh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@buckeyestudio/toh-scope'
import type { CallId, ContentBlock, ToolSchema } from '@buckeyestudio/toh-llm'
import { assertNever, deepFreeze, HarnessError } from '@buckeyestudio/toh-llm'
import type { Agent } from '@buckeyestudio/toh-agent'
import { snapshotJsonValue } from '@buckeyestudio/toh-session'
import type { JsonValue, UserMessage } from '@buckeyestudio/toh-session'
import type { ToolProviderResult } from '@buckeyestudio/toh-system-prompt'
import type { CodeRuntime } from '@buckeyestudio/toh-code-runtime'
 ...
 * instead of deriving them: the seam's `toh-code-runtime` README pair, its
...
export type { JsonValue } from '@buckeyestudio/toh-session'
...
// lives in its own UI-facing module; re-export it so `@buckeyestudio/toh-tools`
...
declare module '@buckeyestudio/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@buckeyestudio/toh-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@buckeyestudio/toh-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@buckeyestudio/toh-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@buckeyestudio/toh-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@buckeyestudio/toh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `toh-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`toh-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`toh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @buckeyestudio/toh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`toh-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `toh-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('toh.tool.execution') as ToolExecutionToken
}
