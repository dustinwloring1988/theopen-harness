 ...
 * @module @buckeyestudio/toh-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@buckeyestudio/toh-llm'
import type { ContentBlock } from '@buckeyestudio/toh-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@buckeyestudio/toh-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`toh-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
