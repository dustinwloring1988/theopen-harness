 ...
 *
 * @module @buckeyestudio/toh-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@buckeyestudio/cordis'
import type { Fiber } from '@buckeyestudio/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@buckeyestudio/toh-scope'
import type { Scoped } from '@buckeyestudio/toh-scope'
import type { SessionEvent, SessionId } from '@buckeyestudio/toh-session'
import type { TypertContext, TypertLookup } from '@buckeyestudio/toh-typert-protocol'
...
declare module '@buckeyestudio/cordis' {
  interface Context {
     ...
     * `toh-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in toh-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `toh-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `toh-agent-loop` package.
 */
 ...
 * (`@buckeyestudio/toh-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@buckeyestudio/toh-agent#Agent',
