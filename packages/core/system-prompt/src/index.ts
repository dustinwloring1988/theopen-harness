 ...
 *
 * @module @buckeyestudio/toh-system-prompt
 */

import { Context, Service } from '@buckeyestudio/cordis'
import z from '@buckeyestudio/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@buckeyestudio/toh-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@buckeyestudio/toh-scope'
import type { ContextSnapshotSection, ToolSchema } from '@buckeyestudio/toh-llm'
...
declare module '@buckeyestudio/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@buckeyestudio/toh-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by TheOpen Harness.',
      })
    }
    this.section({
