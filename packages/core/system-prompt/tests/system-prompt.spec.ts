...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@buckeyestudio/toh-system-prompt'
...
const IDENTITY = 'You are an AI agent powered by TheOpen Harness.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are TheOpen Harness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are TheOpen Harness.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
