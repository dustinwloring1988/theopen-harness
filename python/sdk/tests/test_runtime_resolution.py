...
import theopen_harness_runtime as runtime
import pytest

from theopen_harness_runtime import (
...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@buckeyestudio/toh-agent-spine-demo" in config
    assert "@buckeyestudio/toh-session-persistence-jsonl" in config
    assert "@buckeyestudio/toh-session-checkpoint-policy" in config
...
def test_runtime_requires_spawn_helper_only_on_macos(
    ...
    runtime_dir.mkdir()
    linux = runtime_dir / "toh-jsonrpc-agent-pkg-linux-x64"
    linux.touch()
    Path(f"{linux}-rg").touch()
    macos = runtime_dir / "toh-jsonrpc-agent-pkg-macos-arm64"
    macos.touch()
...
def test_runtime_requires_ripgrep_sidecar(
    ...
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    (runtime_dir / "toh-jsonrpc-agent-pkg-linux-x64").touch()
    monkeypatch.setattr(runtime, "bundled_package_dir", lambda: tmp_path)
    monkeypatch.setattr(runtime, "_current_platform_tag", lambda: "linux-x64")
