...
def test_stage_sdk_keeps_distribution_module_and_runtime_pin_distinct(tmp_path: Path) -> None:
    ...
    assert 'name = "theopen-harness-sdk"' in pyproject
    assert 'version = "1.2.3"' in pyproject
    assert 'license = "MIT"' in pyproject
    assert '"theopen-harness-runtime-bin==1.2.3"' in pyproject
...
def test_stage_runtime_copies_platform_payload(
    ...
    executable = tmp_path / f"toh-jsonrpc-agent-pkg-{target}"
    ...
    assert {path.name: path.read_bytes() for path in runtime_dir.glob("toh-jsonrpc-agent-pkg-*")} == expected
    pyproject = (destination / "pyproject.toml").read_text()
    assert 'license = "MIT"' in pyproject
