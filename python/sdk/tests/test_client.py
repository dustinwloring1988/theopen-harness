...
from theopen_harness import DeepSeekHarness, HarnessClient, HarnessConfig, Notification, SdkProtocolError
...
json.dump({
    ...
    "TOH_CWD": os.environ.get("TOH_CWD"),
    "TOH_SESSION_ROOT": os.environ.get("TOH_SESSION_ROOT"),
    "TOH_CORDIS_CONFIG": os.environ.get("TOH_CORDIS_CONFIG"),
}, open(env_dump, "w"))
...
""".strip()
    ...
    assert dumped_env["TOH_CWD"] == str(tmp_path)
    assert dumped_env["TOH_SESSION_ROOT"] == str(tmp_path / "sessions")
    assert dumped_env["TOH_CORDIS_CONFIG"] == str(tmp_path / "cordis.yml")
...
for line in sys.stdin:
    ...
    if msg.get("method") == "initialize":
        json.dump({"process": os.getcwd(), "environment": os.environ.get("TOH_CWD"), "wire": msg["params"]["cwd"]}, open(os.environ["CAPTURE"], "w"))
        print(json.dumps({"jsonrpc": "2.0", "id": msg["id"], "result": {"serverInfo": {"name": "fake-runtime"}}}), flush=True)
...
""".strip()
    ...
    with HarnessClient(
        ...
        assert init.serverInfo.name == "fake-toh"
...
def test_client_close_keeps_unreaped_runtime_when_kill_join_times_out(
    ...
    # close() marked the client closed even though it kept the unreaped
    # runtime, so start() refuses before any spawn attempt.
...
def test_close_without_runtime_marks_client_closed_so_queued_start_cannot_spawn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from theopen_harness import client as sdk_client

    def forbidden_popen(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("start() must not spawn a runtime after close() returned")

    monkeypatch.setattr(sdk_client.subprocess, "Popen", forbidden_popen)
...
def test_close_bounds_lifecycle_lock_wait_behind_the_shutdown_budget(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from theopen_harness import client as sdk_client
    ...
    client = HarnessClient(
        HarnessConfig(launch_args_override=("unused",), shutdown_timeout_seconds=0.25)
    )
    starter = threading.Thread(target=client.start)
    starter.start()
    assert spawn_entered.wait(timeout=10)

    close_outcome: list[BaseException | None] = [None]
    ...
    assert not closer.is_alive(), "close() blocked past its budget on the lifecycle lock"
    assert isinstance(close_outcome[0], TimeoutError)
    assert elapsed < 5
...
def test_public_signatures_omit_unsupported_wire_parameters() -> None:
    from theopen_harness import DeepSeekHarnessConfig, Session
...
def _install_fake_bundled_runtime(
    ...
    """
    runtime = tmp_path / "toh-jsonrpc-agent"
    runtime.write_text(
        """#!/usr/bin/env python3
import json
import os
...
json.dump({"TOH_CORDIS_CONFIG": os.environ.get("TOH_CORDIS_CONFIG")}, open(os.environ["ENV_DUMP"], "w"))
for line in sys.stdin:
    msg = json.loads(line)
    if msg.get("method") == "initialize":
...
""".strip()
    ...
    module_dir = tmp_path / "theopen_harness_runtime"
    module_dir.mkdir()
...
""".strip()
    ...
    monkeypatch.delitem(sys.modules, "theopen_harness_runtime", raising=False)
...
def test_client_default_launch_uses_bundled_runtime_and_injects_default_config(
    ...
    assert json.loads(env_dump.read_text())["TOH_CORDIS_CONFIG"] == str(default_config)
...
def test_client_respects_explicit_config_over_bundled_default(
    ...
    with HarnessClient(
        HarnessConfig(env={"ENV_DUMP": str(env_dump), "TOH_CORDIS_CONFIG": "./explicit.yml"})
    ) as client:
        client.initialize(provider="deepseek-official", cwd="/workspace", model="deepseek-v4-pro")

    assert json.loads(env_dump.read_text())["TOH_CORDIS_CONFIG"] == "./explicit.yml"
...
def test_client_reports_missing_bundled_runtime_dependency(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(FileNotFoundError, match="Install theopen-harness-runtime-bin"):
        HarnessClient().start()
