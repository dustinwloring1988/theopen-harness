...
`DeepSeekHarness` keeps its lazily started runtime subprocess for reuse across calls. Use it as a context manager, as above, or call `close()` explicitly when finished. `close()` spends the `shutdown_timeout_seconds` budget on the shutdown request and process exit — `None` selects an internal 30-second ceiling instead of an unbounded wait — then escalates from terminate to kill, so the call always returns even against a wedged runtime.
