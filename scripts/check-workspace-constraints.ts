...
const vendoredPackages = new Set([
  '@buckeyestudio/cordis',
  '@buckeyestudio/cosmokit',
  '@buckeyestudio/schemastery',
  '@buckeyestudio/cordis-plugin-loader',
  '@buckeyestudio/cordis-plugin-include',
  '@buckeyestudio/cordis-plugin-group',
  '@buckeyestudio/cordis-plugin-timer',
  '@buckeyestudio/cordis-plugin-hmr',
  '@buckeyestudio/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@buckeyestudio/node-addon-landlock-run',
  '@buckeyestudio/node-addon-landlock-run-linux-arm64',
  '@buckeyestudio/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@buckeyestudio/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/dustinwloring1988/theopen-harness.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/dustinwloring1988/theopen-harness.git'
...
const experimentalPackageNamePrefix = '@buckeyestudio/toh-experimental-'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@buckeyestudio/toh': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@buckeyestudio/toh-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@buckeyestudio/toh-client-ui-primitives': ['lib/**/*.css'],
  '@buckeyestudio/toh-client-web': ['lib/**/*.css'],
  '@buckeyestudio/toh-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@buckeyestudio/toh-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@buckeyestudio/toh-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@buckeyestudio/toh-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // SQLite loads every statement from immutable package resources at runtime.
  '@buckeyestudio/toh-session-persistence-sqlite': ['resources/sql/**/*.sql'],
  '@buckeyestudio/toh-skill-badge': ['assets'],
  '@buckeyestudio/toh-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function expectedTohPackageFiles(manifest: PackageManifest): readonly string[] {
  const declaredPatch = manifest.toh?.bundle?.patch
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the toh family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@buckeyestudio/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@buckeyestudio/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@buckeyestudio/toh-')) {
    const peer = manifest.peerDependencies?.['@buckeyestudio/cordis']
    ...
    const expectedFiles = expectedTohPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the toh release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:toh` and its publication.
