...
import GoalService from '@buckeyestudio/toh-goal'
import SystemPrompt from '@buckeyestudio/toh-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@buckeyestudio/toh-tools'
import LocalBashExecutor from '@buckeyestudio/toh-bash-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@buckeyestudio/toh-attachment'
...
import * as WebSearchExa from '@buckeyestudio/toh-web-search-exa'
import * as WebFetchLocal from '@buckeyestudio/toh-web-fetch-http'
import SubagentRuntime from '@buckeyestudio/toh-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@buckeyestudio/toh-subagent'
import * as ToolSubagentControl from '@buckeyestudio/toh-tool-subagent-control'
import * as ToolSubagentListAgents from '@buckeyestudio/toh-tool-subagent-control/list-agents'
...
import * as ToolPwsh from '@buckeyestudio/toh-tool-pwsh'
...
import CordisHostRunner from '@buckeyestudio/toh-cordis-host-runner'
import * as ToolCordis from '@buckeyestudio/toh-tool-cordis'
import * as ToolFs from '@buckeyestudio/toh-tool-fs'
import * as ToolFsSearch from '@buckeyestudio/toh-tool-fs-search'
import * as ToolStrReplaceEditor from '@buckeyestudio/toh-tool-str-replace-editor'
import TerminalSessionService from '@buckeyestudio/toh-terminal'
import * as ToolPty from '@buckeyestudio/toh-tool-terminal'
import * as ToolGoal from '@buckeyestudio/toh-tool-goal'
import * as ToolSchedule from '@buckeyestudio/toh-schedule'
import Lsp from '@buckeyestudio/toh-lsp'
import * as ToolLsp from '@buckeyestudio/toh-tool-lsp'
import * as ToolSkill from '@buckeyestudio/toh-tool-skill'
import * as ToolSessionQuery from '@buckeyestudio/toh-tool-session-query'
import * as ToolTasks from '@buckeyestudio/toh-tool-jobs'
import type TeamService from '@buckeyestudio/toh-experimental-agent-team'
import * as ToolTeam from '@buckeyestudio/toh-experimental-tool-agent-team'
import * as ToolTodo from '@buckeyestudio/toh-tool-todo'
import * as ToolSubagent from '@buckeyestudio/toh-tool-subagent'
import * as ToolWeb from '@buckeyestudio/toh-tool-web'
import VmWorkflowEngine from '@buckeyestudio/toh-workflow-worker-thread'
import * as ToolRalph from '@buckeyestudio/toh-tool-ralph'
import * as ToolWorkflow from '@buckeyestudio/toh-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@buckeyestudio/toh-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  {
    pkg: '@buckeyestudio/toh-plan-mode',
    dir: 'plan-mode',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-bash',
    dir: 'tool-bash',
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@buckeyestudio/toh-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@buckeyestudio/toh-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@buckeyestudio/toh-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `TOH_*` environment comes from `@buckeyestudio/toh-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@buckeyestudio/toh-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@buckeyestudio/toh-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or TOH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@buckeyestudio/toh-tool-bash-persistent',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-pwsh-persistent',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@buckeyestudio/toh-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@buckeyestudio/toh-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-terminal',
    dir: 'tool-terminal',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-goal',
    dir: 'tool-goal',
  ...
  {
    pkg: '@buckeyestudio/toh-schedule',
    dir: 'schedule',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-lsp',
    dir: 'tool-lsp',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        tohHome: resolve(root, '.tmp/tool-catalog/.toh'),
  ...
  {
    pkg: '@buckeyestudio/toh-tool-session-query',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-subagent-control',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-subagent-report',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-jobs',
    dir: 'tool-jobs',
  ...
  {
    pkg: '@buckeyestudio/toh-experimental-tool-agent-team',
    dir: 'tool-agent-team',
    ...
    scope: ctx => catalogChildScopes.get(ctx) as Agent,
    note:
      'All ten tools are scoped to implicit Team Leads and durable teammates. The shipped toh-base bundle keeps the package disabled; the documented Agent Teams profile patch enables it while disabling the legacy continuable-child control names.',
  },
  {
    pkg: '@buckeyestudio/toh-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-workflow',
    dir: 'tool-workflow',
  ...
  {
    pkg: '@buckeyestudio/toh-tool-web',
    dir: 'tool-web',
