/**
 * Rescope the vendored Cordis packages into the `@buckeyestudio` scope, and undo
 ...
 * `@buckeyestudio/toh-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@buckeyestudio/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@buckeyestudio/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@buckeyestudio/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@buckeyestudio/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@buckeyestudio/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@buckeyestudio/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@buckeyestudio/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@buckeyestudio/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@buckeyestudio/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@buckeyestudio/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@buckeyestudio/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@buckeyestudio\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@buckeyestudio/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@buckeyestudio/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@buckeyestudio/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@buckeyestudio/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@buckeyestudio/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@buckeyestudio/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@buckeyestudio` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@buckeyestudio\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@buckeyestudio/cordis']
    const dev = manifest.devDependencies?.['@buckeyestudio/cordis']

    if (!peer) errors.push(\`\${label}: @buckeyestudio/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @buckeyestudio/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @buckeyestudio/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    id: 'knip-bundle-base',
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@buckeyestudio/\')) {',
    replace: `      // Vendored packages are rescoped too (vendor/README.md), so publication
      ...
      if (!name.startsWith('@buckeyestudio/')) {`,
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@buckeyestudio/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@buckeyestudio/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@buckeyestudio/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@buckeyestudio/` as "another plugin package".
 ...
 */
const VENDORED_LIBRARY = /^@buckeyestudio\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@buckeyestudio/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@buckeyestudio\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@buckeyestudio/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @buckeyestudio, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'toh-packed-consumer', private: true, type: 'module' }))
