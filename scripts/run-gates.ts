...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.TOH_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('TOH_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$TOH_GATE_CONCURRENCY'
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { TOH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('toh-package-licenses', 'verify-toh-package-licenses', { label: 'TOH package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('TOH_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('toh-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'toh source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { TOH_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { TOH_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  const workerRaw = process.env.TOH_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: TOH_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `TOH_SNAPSHOT=replay TOH_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
      env: { TOH_SNAPSHOT: 'replay' },
      needs,
      streamOutput: true,
    })
  }
  return pnpmScript('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
    displayCommand: 'TOH_SNAPSHOT=replay pnpm run test:web:built',
    env: { TOH_SNAPSHOT: 'replay' },
    needs,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.TOH_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `TOH_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// TOH_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// TOH_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set TOH_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// TOH_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test and expect.poll
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('TOH_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.TOH_GATE_VERBOSE === '1'
