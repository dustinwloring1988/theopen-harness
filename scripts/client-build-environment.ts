...
const CLIENT_BUILD_ENV_PREFIX = 'TOH_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'TOH_BUILD_CLIENT_PROFILE'

/** Public client environment required by official TOH artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'TOH_CLIENT_COMMIT_HASH'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.toh-build/client-build-environment.json'
...
): Readonly<Record<`TOH_CLIENT_${string}`, string>> {
  return {
    TOH_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
 ...
 * @returns defined `TOH_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return { TOH_CLIENT_COMMIT_HASH: commitHash, ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`TOH_CLIENT_${string}`, string>>,
): void {
