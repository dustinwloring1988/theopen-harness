/**
 * Generate toh-scope's invariant resolver map from the repository TypeScript
 * Program.
 *
 ...
 * `Events` parameter tuple. Zero matches require `@tohScopeScan unsupported`;
...
class ScopedEventGenerator {
  ...
  render(): string {
    ...
    return [
      '/**',
      ' * Generated scoped-event routing-subject resolvers for toh-scope invariants.',
      ' * Do not edit by hand; run `pnpm run gen-scoped-events`.',
      ' *',
      ' * @module @buckeyestudio/toh-scope/scoped-events.generated',
      ' */',
      '',
        ...
        if (ts.isInterfaceDeclaration(node) && node.name.text === 'Events' && isCordisModuleInterface(node)) {
          for (const member of node.members) {
            ...
            if (!scopedBase) {
              ...
              if (tag.present) {
                this.violations.push(`${where} has @tohScopeScan metadata but is not a Scoped event`)
              }
              continue
            }
            if (!raw.includes(SCOPE_DOC_MARKER)) {
            ...
            if (candidates.length === 0) {
              if (!tag.unsupported) {
                ...
                this.violations.push(
                  ...
                  + 'add @tohScopeScan unsupported only when the key is intentionally absent from the payload',
                )
              }
            ...
            if (tag.unsupported) {
              this.violations.push(
                `${where} has unnecessary @tohScopeScan unsupported; ${candidates[0]?.path} exposes the routing key`,
              )
              continue
            }
...
/** Return whether an Events interface is inside declare module '@buckeyestudio/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@buckeyestudio/cordis'
}
...
/** Parse and validate the optional @tohScopeScan unsupported tag. */
function parseScopeTag(raw: string, where: string, violations: string[]): ScopeTag {
  const tags = raw
    ...
    .filter(line => line.startsWith('@tohScopeScan'))
  if (tags.length > 1) violations.push(`${where} has multiple @tohScopeScan tags`)
  if (tags.length === 0) return { present: false, unsupported: false }
  const unsupported = tags[0] === '@tohScopeScan unsupported'
  if (!unsupported) {
    violations.push(
      `${where} has invalid scoped-event scan metadata '${tags[0]}'; expected '@tohScopeScan unsupported'`,
    )
  }
...
/** Generate or freshness-check the fixed toh-scope source file. */
function main(): void {
