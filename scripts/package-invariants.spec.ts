...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'toh-package-invariants-'))
  roots.push(root)
  const dir = join(root, 'packages/core/probe')
  mkdirSync(join(dir, 'src'), { recursive: true })
  const packageName = options.packageName ?? '@buckeyestudio/toh-probe'
  const manifest = {
    name: packageName,
    ...
    peerDependencies: options.invariantDependency === false ? {} : {
      '@buckeyestudio/toh-invariants': 'workspace:^',
    },
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@buckeyestudio/toh-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@buckeyestudio/toh-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@buckeyestudio/toh-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@buckeyestudio/toh-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@buckeyestudio/toh-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@buckeyestudio/toh-probe'
