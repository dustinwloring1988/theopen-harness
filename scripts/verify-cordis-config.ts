 ...
 * silently changes composition. Example configs and the toh Web composition
...
// These example files are overlays consumed by the built toh app, so their bare
...
const appOverlayFiles = new Set([
  ...
  'examples/web-storage-sqlite/cordis.yml',
...
const CHOOSER_PACKAGE = '@buckeyestudio/toh-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  ...
  '@buckeyestudio/toh-client-ui-directory-picker-browse',
  '@buckeyestudio/toh-client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 * `toh.client` block, and the node half of a surface plugin is an empty
 ...
 * `toh.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.toh?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no toh.client, so its browser half is never served`
      : `${manifestPath}: declares toh.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `TOH_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@buckeyestudio/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // toh app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.toh?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).toh?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `toh` source
