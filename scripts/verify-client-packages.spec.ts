...
const CORDIS = '@buckeyestudio/cordis'
const roots: string[] = []
...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@buckeyestudio/toh-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@buckeyestudio/toh-a/subpath'",
      "declare module '@buckeyestudio/toh-client-ui-slots' {}",
      "const load = () => import('@buckeyestudio/toh-b')",
    ...
    expect([...uses].sort()).toEqual([
      '@buckeyestudio/toh-a',
      '@buckeyestudio/toh-b',
      '@buckeyestudio/toh-client-ui-slots',
      'react',
    ])
...
describe('package modes', () => {
  ...
  it('requires seeded workspace packages to use staticLinked and preloads to name dynamic rows', () => {
    ...
    expect(found.join('\n')).toContain('has no dynamic toh.client row')
  })
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@buckeyestudio/toh-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@buckeyestudio/toh-client-feature'],
      sourceUses: {
        '@buckeyestudio/toh-agent': ['packages/client/runtime/src/index.ts'],
        '@buckeyestudio/toh-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@buckeyestudio/toh-agent': 'workspace:^',
        '@buckeyestudio/toh-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@buckeyestudio/toh-agent': 'workspace:^',
        '@buckeyestudio/toh-client-feature': 'workspace:^',
        '@buckeyestudio/toh-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@buckeyestudio/toh-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@buckeyestudio/toh-agent': 'workspace:^' },
    ...
    expect(found).toHaveLength(2)
    expect(found.join('\n')).toContain('peer-installed TOH relationship')
    expect(found.join('\n')).toContain('static client input')
  })

  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@buckeyestudio/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@buckeyestudio/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@buckeyestudio/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@buckeyestudio/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
        react: ['packages/client/web/src/seed.ts'],
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@buckeyestudio/cordis-plugin-loader': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@buckeyestudio/toh-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@buckeyestudio/toh-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@buckeyestudio/toh-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@buckeyestudio/toh-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@buckeyestudio/toh-client-slots/client'] })
  ...
  it('rejects an explicit baseline request', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      ui.manifest + ': toh.client.external repeats baseline module "react"; remove the explicit declaration',
    ])
  })

  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@buckeyestudio/toh-client-ui', '@buckeyestudio/toh-missing', '@buckeyestudio/toh-missing'],
      inject: ['', '@buckeyestudio/toh-a', '@buckeyestudio/toh-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [ui] }))
    expect(found).toHaveLength(6)
    expect(found.join('\n')).toContain('toh.client.external contains an empty value')
    expect(found.join('\n')).toContain('toh.client.inject contains an empty value')
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@buckeyestudio/toh-client-b'],
      inject: ['@buckeyestudio/toh-client-b'],
    })
    const b = declaration('b', {
      external: ['@buckeyestudio/toh-client-a'],
      inject: ['@buckeyestudio/toh-client-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [a, b] }))
    expect(found).toHaveLength(1)
    expect(found[0]).toContain('synchronous toh.client.external cycle')
  })
})
...
describe('manifest declarations', () => {
  it('reports malformed arrays without hiding other packages', () => {
    ...
    const files: Record<string, unknown> = {
      'packages/g/a/package.json': {
        name: '@f/a', toh: { client: { external: 'react', inject: ['@f/b', 1] } },
      },
      'packages/g/b/package.json': { name: '@f/b', toh: { client: {} } },
    }
    ...
    expect(result.malformed).toEqual([
      'packages/g/a/package.json: @f/a toh.client.external must be a string array',
      'packages/g/a/package.json: @f/a toh.client.inject must be a string array',
    ])
  })
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@buckeyestudio/toh-client-feature', '@buckeyestudio/toh-missing'],
      inject: ['', '@buckeyestudio/toh-agent', '@buckeyestudio/toh-agent'],
      sourceUses: {
        '@buckeyestudio/toh-agent': ['packages/client/feature/src/index.ts'],
        '@buckeyestudio/toh-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@buckeyestudio/toh-agent': 'workspace:*',
    ...
    const manifest = {
      name: subject.name,
      toh: { client: { external: subject.external, inject: subject.inject, platform: 'web' } },
    ...
    const fixed = JSON.parse(readFileSync(join(root, subject.manifest), 'utf8')) as {
      toh: { client: { external: string[]; inject: string[] } }
      dependencies?: Record<string, string>
      peerDependencies: Record<string, string>
      devDependencies: Record<string, string>
    }
    expect(fixed.toh.client).toMatchObject({
      external: ['@buckeyestudio/toh-missing'],
      inject: ['@buckeyestudio/toh-agent'],
    })
