 ...
 * itself (`toh web`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/web/dist`, which `toh web`
...
/** Compile-shell workspace whose dist `toh web` serves. */
const SHELL_PACKAGE = '@buckeyestudio/toh-web-frontend'
 ...
 * whose package.json carries `toh.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      toh?: { client?: { platform?: unknown } }
    }
    if (manifest.toh?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `toh.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.toh?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('dev-web: no toh.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} toh.client plugin packages`
