...
const SESSION_PACKAGE = '@buckeyestudio/toh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@buckeyestudio/toh-session/types'
 ...
 * top-level declaration (in `@buckeyestudio/toh-session`) and any declaration
 * merge inside a `declare module '@buckeyestudio/toh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @buckeyestudio/toh-session/known-event-types',
    ' */',
    '',
    '/**',
