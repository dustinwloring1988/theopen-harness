...
const MERGE_HEAD = /declare module ['"](?:@buckeyestudio\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@buckeyestudio/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@buckeyestudio/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@buckeyestudio/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@buckeyestudio/cordis'` Context merge
...
/**
 * Every event name a `declare module '@buckeyestudio/cordis'` Events merge declares in one
