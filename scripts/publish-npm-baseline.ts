...
const RELEASE_ENTRY_PACKAGE = '@buckeyestudio/toh'
...
while time.monotonic() < deadline:
    ...
    if not termination_sent and b"toh web: http://127.0.0.1:" in snapshot:
        ready_seen = True
...
if not ready_seen:
    sys.stderr.write("installed toh web did not reach its ready URL\n")
    sys.exit(124)
actual_exit = os.waitstatus_to_exitcode(status)
if actual_exit != 0:
    sys.stderr.write(f"installed toh web exited {actual_exit}, expected 0\n")
    sys.exit(125)
`
...
class DetachedWorktree {
  ...
  static create(repositoryRoot: string, commit: string, runner: CommandRunner): DetachedWorktree {
    const temporaryRoot = mkdtempSync(join(tmpdir(), 'toh-npm-baseline-'))
  ...
  static discover(root: string): WorkspacePackageSet {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@buckeyestudio/')) {
        throw new Error(`${manifestPath} must name an @buckeyestudio package`)
      }
      if (name === '@buckeyestudio/toh-root') {
...
/** Installs one complete bundle outside the workspace and probes the shipped toh entry. */
class InstalledBundleSmoke {
  ...
  run(): void {
    const consumerRoot = mkdtempSync(join(tmpdir(), 'toh-npm-consumer-'))
    try {
      ...
      writeFileSync(resolve(consumerRoot, 'package.json'), `${JSON.stringify({
        name: 'toh-npm-baseline-consumer',
        version: '0.0.0',
        private: true,
        dependencies,
      }, null, 2)}\n`)
      ...
      const bin = resolve(consumerRoot, 'node_modules/@buckeyestudio/toh/lib/bin.js')
      assertPathWithin(consumerRoot, bin, 'installed toh bin')
      ...
      if (version !== this.bundle.manifest.version) {
        throw new Error(
          `installed toh --version returned ${JSON.stringify(version)}; `
          + `expected ${this.bundle.manifest.version}`,
        )
      }
      this.probeWeb(bin, consumerRoot, environment)
      console.log('publish-npm-baseline: installed toh entry and Web startup probes passed')
    } finally {
  ...
  private probeWeb(bin: string, consumerRoot: string, environment: NodeJS.ProcessEnv): void {
    if (process.platform === 'win32') {
      throw new Error('installed toh Web probe requires a POSIX host with python3')
    }
    const result = this.runner.result(
      'python3',
    ...
    if (result.status !== 0) {
      throw commandFailure('python3', ['installed-toh-web-probe'], result)
    }
  }
}
...
function parsePackedPackage(value: unknown, index: number): PackedPackage {
  ...
  if (origin === 'harness' && (!name.startsWith('@buckeyestudio/') || name === '@buckeyestudio/toh-root')) {
...
function installedArtifactEnvironment(consumerRoot: string): NodeJS.ProcessEnv {
  ...
  environment.TOH_HOME = resolve(consumerRoot, '.toh')
  environment.TOH_AGENTS_HOME = resolve(consumerRoot, '.agents')
  environment.TOH_TELEMETRY_DISABLED = '1'
