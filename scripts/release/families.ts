 ...
 * owns: `toh` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@buckeyestudio/toh-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@buckeyestudio/')) throw new Error(`${normalized} must name an @buckeyestudio package`)
...
class TohFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`toh release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@buckeyestudio/toh', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@buckeyestudio/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new TohFamily(), new VendorFamily()]
}
