 ...
 *
 * The toh family shares one version across its publishable members, private
...
/** Release types the toh family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the toh family's version. */
...
/** One private toh package whose version follows the publishable family. */
interface PrivateTohVersion {
  /** Repository-relative manifest path. */
  readonly manifestPath: string
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:toh <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package manifests that share the toh version without joining
 * its publish set.
 * @param root - repository root.
 * @returns Private package manifests sorted by path.
 */
function privateTohVersions(root: string): PrivateTohVersion[] {
...
/**
 * Plan the toh family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the toh family.
 * @param root - repository root.
...
export function planShared(
  ...
  for (const entry of privateTohVersions(root)) {
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <toh|vendor> [version]')
  ...
  if (family.id === 'toh') {
    ...
    if (request === undefined) throw new Error('usage: release:toh <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:toh takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
