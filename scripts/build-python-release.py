...
SDK_DISTRIBUTION = "theopen-harness-sdk"
RUNTIME_DISTRIBUTION = "theopen-harness-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="toh-python-release-") as temporary:
        staging = Path(temporary) / args.package
        if args.package == "sdk":
            stage_sdk(staging, wheel_version)
            environment = None
            expected = output_dir / f"theopen_harness_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            environment = {"TOH_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"theopen_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"theopen-harness-runtime-bin==[^"]+"',
        f'"theopen-harness-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "theopen_harness_runtime" / "runtime"
