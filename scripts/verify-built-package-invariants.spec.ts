...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'toh-built-package-invariants-'))
  roots.push(root)
  const packageDir = join(root, 'packages/core/probe')
  mkdirSync(join(packageDir, 'lib'), { recursive: true })
  writeFileSync(join(packageDir, 'package.json'), `${JSON.stringify({
    name: '@buckeyestudio/toh-probe',
    type: 'module',
...
describe('built package invariant verifier', () => {
  ...
  it('rejects a default export and a broken invariant export map', () => {
    ...
    expect(exportResult.stderr).toContain('@buckeyestudio/toh-probe')
  })
