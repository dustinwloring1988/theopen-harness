 ...
 * the REAL `toh-jsonrpc-agent` runtime (per `TOH_EXAMPLE_MODE`) through the
 * REAL `@buckeyestudio/toh-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `TOH_SNAPSHOT=record`
 * re-records against the live API; `TOH_SNAPSHOT=refresh` replays committed
...
const mode = process.env.TOH_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo toh-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { TOH_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    TOH_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    TOH_SESSION_ROOT: sessionsRoot,
    TOH_CWD: cwd,
    TOH_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      TOH_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { TOH_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
