...
import { resolvePwshPath } from '@buckeyestudio/toh-pwsh-local'
import { parseSessionLog } from '@buckeyestudio/toh-llm-replay'
import { OFFLOADED_IMAGE_TEXT } from '@buckeyestudio/toh-llm'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `toh-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
// The toh-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
async function prepareEditingCordisSkillWorkspace(cwd: string): Promise<void> {
  const target = join(cwd, '.toh', 'skills', 'editing-cordis-compositions', 'SKILL.md')
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown TOH_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { TOH_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      TOH_PERMISSION_MODE: 'read-only',
      TOH_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // (XXX(concurrent-subagents) in toh-llm-replay).
  ...
  // published-handle disposal failure.
  {
    ...
    env: { TOH_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { TOH_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.TOH_SNAPSHOT),
  hasPwsh,
})
...
it('pins native DeepSeek Files offload and inline fallback in assembled requests', async () => {
  ...
  try {
    const result = await runScenario(input, {
      ...
      env: {
        TOH_SNAPSHOT_API_KEY: 'snapshot-key',
        TOH_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
    ...
    const fallback = await runScenario(input, {
      ...
      workspaceDir: join(SNAPSHOTS_DIR, 'read-image', 'workspace'),
      env: {
        TOH_SNAPSHOT_API_KEY: 'snapshot-fallback-key',
        TOH_SNAPSHOT_BASE_URL: `http://127.0.0.1:${address.port}`,
      },
    })
