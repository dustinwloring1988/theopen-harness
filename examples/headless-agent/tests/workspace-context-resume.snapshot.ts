...
import { renderWorkspaceContext } from '@buckeyestudio/toh-agent-instructions'
import { resolveConfig, workspaceBaselineIdentity } from '@buckeyestudio/toh-agent-instructions/src/config.ts'
...
describe('agent-instructions resume snapshot', () => {
  it('appends an offline replacement without duplicating the visible baseline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'toh-workspace-context-resume-',
      binScript,
      ...
      tsconfigPath,
      env: {
        TOH_SNAPSHOT_FILE: replayFixture,
  ...
  it('supersedes an incompatible baseline when precedence changed offline', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'toh-workspace-context-precedence-',
      binScript,
      libBinScript: binScript,
      configPath,
      binArgs: [configPath, 'Acknowledge the current workspace instruction.'],
