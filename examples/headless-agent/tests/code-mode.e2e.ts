...
import LlmRuntime, { createUserMessage, CallId, HarnessError  } from '@buckeyestudio/toh-llm'
...
import ToolRuntime, { RUN_CODE_NAME, defineTool } from '@buckeyestudio/toh-tools'
import type { ToolExecutionResult } from '@buckeyestudio/toh-tools'
...
import { WorkerThreadCodeRuntime } from '@buckeyestudio/toh-code-runtime-worker-thread'
...
async function workspaceCodeModeHarness(): Promise<Context> {
  ...
  await harness.plugin(AgentLoop, { agents: [] })
  await harness.plugin(LlmDeepSeek, { models: [{ id: FLASH }] })
  await harness.plugin(WorkerThreadCodeRuntime, {})
  return harness
}
...
describe('Code Mode typed values: keyless real-worker contracts', () => {
  ...
  it('returns a background job id, settles the outer run, and polls that id to completion', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'toh-code-mode-background-'))
  ...
  it('pre-abort spawns nothing; post-publication abort leaves job_kill as the cancellation owner', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'toh-code-mode-task-cancel-'))
    ctx = await backgroundCodeModeHarness(workdir)
  ...
  it('keeps foreground bash coupled to the outer signal', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'toh-code-mode-foreground-cancel-'))
    ctx = await backgroundCodeModeHarness(workdir)
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('Code Mode: real model writes a program over real tools', () => {
  it('collapses the wire tool list to [run_code], bridges sub-calls, and returns curated output', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'toh-code-mode-e2e-'))
    ctx = await codeModeHarness(workdir)
    const agent = ctx.agentLoop.create(SessionId('e2e-code-mode'), { provider: 'deepseek-official', model: FLASH })
  ...
  it('projects nested workspace instructions discovered by an fs sub-call', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'toh-code-mode-workspace-e2e-'))
    ...
    const handle = await ctx.agents.create({
      ...
      meta: { cwd: workdir },
      agentOptions: { provider: 'deepseek-official', model: FLASH },
    })
