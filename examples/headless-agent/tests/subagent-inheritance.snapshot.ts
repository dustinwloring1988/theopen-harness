...
import { normalizeSessionSnapshot, type NormalizeContext } from '@buckeyestudio/toh-acp-snapshot'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId, type SessionEvent, type SessionHeader } from '@buckeyestudio/toh-session'
...
describe('parent-only override inheritance snapshot', () => {
  it('confines a delegated child through the assembled headless app', async () => {
    ...
    const result = await runLoaderSmoke({
      ...
      tempDirPrefix: 'toh-subagent-inherit-',
      binScript,
      ...
      env: {
        ...
        TOH_SNAPSHOT_FILE: replayOverride,
        TOH_SNAPSHOT_OVERRIDE: replayOverride,
      ...
      inspect: async (runCwd) => {
        ...
        const runtimeContexts = (content: string): string[] => content.trimEnd().split('\n').flatMap((line) => {
          ...
          if (record.type !== 'user/message'
            ...
            || record.data.source.plugin !== '@buckeyestudio/toh-system-prompt') return []
        ...
        for (const context of policyContexts) {
          expect(context).toContain('Any available operation enforced by the TOH file sandbox cannot modify files in the standing mode.')
