...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `TOH_*` environment vocabulary |
...
| [memory.md](memory.md) | the cross-session fact memory seam: `MemoryFact`/`MemoryFactId`, provider registration and selection, workspace-scoped recall and delete, and the `memory/changed` event |
...
| [client-modules.md](client-modules.md) | the web plugin table: `toh.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
