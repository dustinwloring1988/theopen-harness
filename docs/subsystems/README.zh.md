...
每个子系统一页，覆盖 TheOpen Harness 的全部子系统：它是什么、它操作哪些数据结构，以及——当它由某个 `ctx` 服务或事件作用域支撑时——一段生成的 **Cordis API** 小节，承载其服务与事件参考。本目录与 [architecture.md](../architecture.zh.md) 互补：后者描述跨子系统的*行为*（服务映射、会话/轮次/步骤生命周期、事件分类体系）；这里的每一页是单个子系统词汇与接线的参考。
...
| [subprocess.md](subprocess.zh.md) | 子进程 seam：完全显式的 `SubprocessSpawnSpec`、基于偏移的输出读取器、不含分类的 `SubprocessOutcome`，以及受管 `TOH_*` 环境词汇 |
...
| [memory.md](memory.zh.md) | 跨会话事实记忆 seam：`MemoryFact`/`MemoryFactId`、提供方注册与选择、按工作区 scope 的召回与删除，以及 `memory/changed` 事件 |
...
| [client-modules.md](client-modules.zh.md) | Web 插件表：`toh.client` 声明、`WebBootGraph` 线上组合、bundle 路由与 index 转换 |
