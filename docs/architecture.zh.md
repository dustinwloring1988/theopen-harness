...
[Cordis](cordis-primer.zh.md) 是 toh 底层的框架：插件向共享上下文贡献服务、类型化事件和可逆的副作用。产品的每一部分都是插件，包括模型适配器、工具注册表、会话日志，以及 agent loop（智能体循环）本身，因此每一部分都可以从配置替换。

不存在需要打补丁的特权内核：扩展 toh 的方式是把插件挂载到其他插件旁边，而各项注册都是副作用，会在其插件卸载时撤销。

## Profile 与组合包

运行中的 `toh` 是一棵插件树，由启动时按序叠加的各层组合而成。
...
两者都在各自的 `package.json` 中通过 `toh` 字段声明自己：`toh.profile` 列出一个 profile 的组合包，`toh.bundle` 指向一个组合包的 patch 文件。

[`toh-base`](../packages/bundle/base/README.zh.md) 是每个 profile 的第一层：模型适配器、工具、持久化、沙箱与审批策略、设置、凭据、遥测。[`toh-web-app`](../packages/bundle/web-app/README.zh.md) 增加浏览器应用；[`toh-headless`](../packages/bundle/headless/README.zh.md) 增加一次性运行器，且完全不带服务器。
...
一个**步骤**是一次模型请求加上它调用的工具。一个**轮次**包含零个或多个步骤：它在领取首条输入之前打开，并在不再欠下任何工作时关闭（[loop 层级定义](glossary.zh.md#loop-hierarchy)）。
...
一个 **seam** 是一项可替换能力，包含三种角色：**Service Definition**（拥有自身 `ctx.<key>` 和词汇类型的 Cordis `Service`）、实现它的 **Service Provider**，以及使用它的 **Consumer**（通常是面向模型的工具）。一个包可以合并承担多个角色，但单一角色本身不是 seam；添加一项能力意味着把三者一并设计（[能力图](capability-seams.zh.md)；[规范定义](glossary.zh.md#capability-seam)）。
...
| 让某个会话拥有不同的能力集合 | 组装一个 agent preset；其中的服务行需要 [`isolate` realm](cordis-primer.zh.md#scopes-and-realms) |
| 添加 shell 执行 | 注册 `ctx.shell` 后端；本地后端通过 `ctx.subprocess` spawn 进程 |
| 添加持久化终端执行 | 注册 `ctx.terminals` 后端和 `toh-tool-terminal` |
