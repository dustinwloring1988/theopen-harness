...
- **Service Definition** (`toh-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`toh-bash-local`) — executes commands on the local machine
- **Consumer** (`toh-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  toh-shell   │────▶│  toh-bash-local  │     │ toh-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@buckeyestudio/toh-my-cap'
...
```yaml
- name: '@buckeyestudio/toh-my-cap-local'
- name: '@buckeyestudio/toh-tool-my-cap'
```
