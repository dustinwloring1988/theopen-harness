...
密钥是只写的。保存后，页面只会收到脱敏描述符，永远不会收到明文密钥。密钥存储在 `$TOH_HOME/.credentials.yaml` 中，settings 只保留它的凭据引用。
...
因此自定义提供方下的视觉模型需要加一行。表单没有对应字段；请在 `$TOH_HOME/settings.yaml` 中给该模型加上 `input`：
...
其中两样占了绝大多数。声明了推理能力的模型，其系统提示词会以 `role: "developer"` 发出，很多网关直接拒绝；输出上限则写作 `max_completion_tokens`，只认 `max_tokens` 的服务端会拒绝。表单里没有这两个字段；请在 `$TOH_HOME/settings.yaml` 的路由上更正：
...
全部开关、各自接受的取值，以及接受它们的协议，都列在[生成的 `toh-llm-pi-ai` 配置参考](../../config-catalog.zh.md#buckeyestudiotoh-llm-pi-ai)的 `PiAiCompatProfile` 之下——该参考派生自源码，因此不会落后于适配器实际接受的内容。
...
自动生成的[插件配置目录](../../config-catalog.zh.md)列出每个插件的所有受支持字段与默认值；[`toh-llm-pi-ai`](../../config-catalog.zh.md#buckeyestudiotoh-llm-pi-ai) 就是本页所配置的那个提供方段落。[`toh-llm-pi-ai`](../../../packages/llm/llm-pi-ai/README.zh.md) 和 [`toh-llm-deepseek`](../../../packages/llm/llm-deepseek/README.zh.md) 参考文档负责直接 `settings.yaml` 配置、目录解析、推理控制、凭据与适配器错误。
