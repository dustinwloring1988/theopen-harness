...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-local`](../packages/memory/memory-local) | [`tool-memory`](../packages/memory/tool-memory) | - | 提供方拥有事实存放在哪里、查询如何匹配（当前为关键词子集）；注册表拥有提供方映射、执行期选择与 `memory/changed` 事件的发出，tool-memory 把 remember/recall/forget 暴露给模型。 |
