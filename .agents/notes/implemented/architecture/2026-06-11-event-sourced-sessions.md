...
- Persistence stays a plugin concern; the in-memory store ships in toh-session.
...
- Derivation cost grows with log length — compaction (toh-compaction) is the intended mitigation, not log mutation.
