...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `toh-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `toh-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `toh-timeout` (`idleWatchdog`) |
| Clear the timer | `toh-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `toh-timeout` (`timeoutOf`) |
