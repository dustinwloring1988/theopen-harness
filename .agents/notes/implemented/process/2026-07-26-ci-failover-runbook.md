...
`toh-win-ci`: 32 always-on runner instances (scheduled tasks `GH-Runner-01`…`GH-Runner-32`) on the in-house Windows CI server (one 96-core / 580 GB machine). Labels: `[self-hosted, toh-win-ci, windows]`. The image must preinstall Node 24, pnpm, Git (with Git Bash on `PATH`, i.e. `C:\Program Files\Git\bin` — the `bash` tool spawns `bash` by name), PowerShell 7, and enable Developer Mode for symlink support. Check the latest `serial / windows (self-hosted standby)` run before switching: a green standby verifies the pool can execute `check:ci:windows-complete` end-to-end.
...
1. Repository **Settings → Secrets and variables → Actions → Variables → New repository variable**: name `TOH_CI_FAILOVER_LINUX` (Linux pool outage) or `TOH_CI_FAILOVER_WINDOWS` (Windows pool outage), value `selfhosted`.
...
3. That is the entire switch. Under Linux failover the workflow also drops `TOH_SNAPSHOT_MAX_CONCURRENCY` to 12 for the shared VM and skips the hosted-path pnpm cache restores because the VM's persistent store serves warm installs. Coverage uses the same four single-worker instrumented partitions and two exempt workers on both Linux pools. The Windows switch has no concurrency or cache branches; it only retargets the native Windows job's pool.

#**Untrusted-PR exception (Dependabot and forks).** Both switches' selectors deliberately exclude `dependabot[bot]` PRs and fork PRs (`github.event.pull_request.head.repo.fork == false`, mirroring the proven e2e fork gate): under failover those pull requests stay queued for the hosted pool rather than executing untrusted code on the persistent VMs and their durable caches. An excluded pull request that remains queued during an outage is expected behavior, not a failed switch; it completes when the hosted pool recovers.
