...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `toh-tool-bash` (`bash`), `toh-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `toh-tool-subagent` (`subagent`). Example-only tools are excluded.
