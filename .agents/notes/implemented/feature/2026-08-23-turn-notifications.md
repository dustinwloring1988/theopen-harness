...
The first ready snapshot of any watcher records standing facts without notifying (the SessionManager completed-reminder precedent): a page reloading into an already-blocked world neither prompts nor toasts, and the pre-baseline `pending` phase arms no keys either. Settings live in the `ui-notify` namespace registered by the plugin's node half, edited through the General-section row this package also registers (`settings.general.item`, id `notifications`). The row publishes gestures locally before the durable write settles, so memory-mode compositions see the choice this session. Nothing here reaches a model request or the session log.
...
- **Raising approval toasts even inside quiet hours** — rejected for symmetry: quiet hours suppress approvals like everything else, so approval notifications appear only outside the window and the sidebar dots remain the escalation path there; uninterrupted approval notifications call for disabling or shrinking the quiet window, not for switching the mode.
