...
- The commands package now depends on `toh-attachment` and `toh-llm`, and `commands/execute` carries a required `images` wire parameter — every caller states its envelope explicitly.
