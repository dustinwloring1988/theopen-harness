...
**One existing client Cordis plugin owns the shipped step.** `ui-settings-models` registers `welcome-notice` at order `-100` in `settings.onboarding` (the official-DeepSeek credential step registered beside it at order `0` was removed by [dormant official DeepSeek provider](2026-08-21-dormant-official-deepseek-provider.md)). The shell continues to mount only the first incomplete entry, so dialogs cannot stack. No additional client package or plugin row is introduced.
...
A fresh loopback profile sees the specified internal-testing notice; the inline DeepSeek key dialog was later removed outright ([dormant official DeepSeek provider](2026-08-21-dormant-official-deepseek-provider.md)). Acknowledgement remains versioned in `settings.yaml`, secrets remain write-only in `.credentials.yaml`, and already-ready or unsupported deployments render no onboarding chrome while readiness loads. The Models package now owns product-onboarding presentation as well as provider configuration; its README and browser coverage make that broader responsibility explicit. This decision restores a concise testing-stage notice after the historical [full-viewport beta notice removal](../simplification/2026-08-13-remove-first-run-beta-notice.md) without restoring that notice's telemetry copy or takeover layout.
