...
Every same-origin redirect hop re-enters the request path, so each hop re-resolves and re-validates; cross-origin redirects were already refused (`WEB_REDIRECT_BLOCKED`). When a destination is blocked, the provider raises its new error code `WEB_PRIVATE_NETWORK_BLOCKED`.
