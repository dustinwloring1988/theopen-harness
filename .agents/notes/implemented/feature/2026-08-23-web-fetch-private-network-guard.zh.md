...
分类测试覆盖每个具名范围家族加上紧贴各前缀外侧的边界地址、公开对照、格式错误输入（包括带前导零的八位组，它们保守失败）、mapped／compatible／NAT64／6to4 内嵌（内嵌目标被阻止与公开两种情形）以及本地名称识别。策略测试证明：多条地址结果中任一记录非公开即阻止；benchmarking 与 CGNAT 目标默认阻止、显式选择加入后放行；本地名称不经解析器即拒绝；字面 IP 永不触及解析器；每次调用都会重新解析——这正是重定向跳所依赖的性质。
