...
Unit (`tests/prompts.spec.ts`, mocked SDK): policy resolution, slug mapping, candidate mapping including pagination, fallback description, invocation-policy propagation, lazy `prompts/get` loading with argument templating and role-tagged rendering, unloadable-body reporting, contained slug collisions with recovery through the notification handler, notification-driven resync, resync riding a reconnect generation with stale-handler inertness, budget exhaustion emptying the catalog, default-off wiring, loud failure without a registry, and disposal unregistering the provider. Snapshot: none — bridging adds no new presentation shape, and the surface exists only under explicit opt-in configuration.
