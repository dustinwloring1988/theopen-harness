...
[`session-title` 能力包族](../../../../packages/session/README.zh.md)负责标题状态和生成策略。`@buckeyestudio/toh-session-title` 提供 `ctx.sessionTitle`、确定性的首消息回退方案，以及一个至多接受单个可选异步提供方的注册表。`@buckeyestudio/toh-session-title-llm` 是唯一的模型驱动提供方插件；其必填且经过验证的 `cadence` 配置选择首条提示词或全部提示词的消息选取方式。共享 agent 主干只挂载回退服务。Web host 会挂载该服务和 `first-prompt` 节奏的模型提供方，并显式设置可覆盖的限制，因此新建的 Web 会话会立即获得回退标题，随后在不阻塞主响应的情况下获得模型摘要。其他组合需以任一节奏显式挂载该提供方。
...
`first-prompt` 节奏下，提供方仅在新会话首次创建回退标题时调度一次；自动执行失败后，后续提示词不会重新调度，`refresh()` 是重试路径。`all-prompts` 节奏下，它会在每条合格且由人类发出的提示词后调度，并传入截至该修订的所有合格消息，包括预置历史记录。任一节奏下，较新的修订都会中止并取代更早的待执行或活跃工作。
...
与源日志的其他部分相同，fork 会原样继承作为种子的标题事件——被钉住（user 来源）的标题在子会话中保持钉住，直到显式 refresh。`first-prompt` 节奏不会自动为 fork 重新生成标题。`all-prompts` 节奏可以在子会话出现后续提示词后追加一项归子会话所有的修订，并使用继承的合格消息和新增的合格消息。
