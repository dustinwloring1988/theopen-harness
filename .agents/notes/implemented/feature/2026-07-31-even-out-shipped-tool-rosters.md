...
**`toh-tool-cordis`** lets the model write JavaScript and mount it as a temporary plugin. Its README states the limit: "The sandbox is containment for honest code, not a security boundary — host-realm helpers on the sandbox global are reachable, so mount code can reach Node" ([Known limitations](../../../../packages/extensions/tool-cordis/README.md)). The `node:vm` realm lives inside the harness process while `toh-sandbox-local` confines only the argv it spawns, so on the Web surface both the sandbox and the approval seam are bypassed rather than enforced.

**`toh-web-fetch-http`** was unmounted and `toh-tool-web` kept `fetch: false` when this roster landed, because SSRF protection was deferred in the implementation ([`policy.ts`](../../../../packages/web/web-fetch-http/src/policy.ts) validated protocol, credentials, and length only). The model chooses the target, which includes the harness's own gateway on loopback, private ranges, and cloud metadata endpoints. The [private-network guard decision](2026-08-23-web-fetch-private-network-guard.md) since supplied that protection — DNS-resolve-then-validate with connection pinning, per-hop re-validation, and `allowPrivateNetworks: false` by default — so both rows now mount enabled in the shared base.

Withholding it had narrowed the surface without removing the reach: `bash` is mounted, so `curl` gets the same page. What the absence bought was the removal of an argument-shaped request primitive that needs no shell — and with it the accidental path where a summarization request quietly reaches loopback. A deployment that must contain outbound traffic still needs a network-level control; the guard blocks private destinations, not egress from a compromised process.
...
`@buckeyestudio/toh-mcp-client` becomes a runtime dependency of the CLI without a row in any shipped config. The plugin mounts exactly one server per instance and `command` is required, so a default would have to name a third-party server and spawn it as a child process on every launch — outside `ctx.shell`, and therefore outside the sandbox policy the Web surface composes.

The layer that would make MCP a default is the one this repository does not have yet: a bridge that reads a user's server list and mounts one client per entry, the same shape [`toh-hooks-claude-code`](../../../../packages/hooks/hooks-claude-code/README.md) already has for a Claude Code `hooks.json`. Shipping the dependency means an installed `toh` can mount servers from `$TOH_HOME/config.yaml` today; the CLI README carries the YAML.
...
`glob` and `grep` are asserted as fixed members rather than a host-dependent pair: `toh-tool-fs-search` spawns the packaged ripgrep binary and registers both tools unconditionally, so the pair is always present.
...
`apps/cli` gained five workspace dependencies: four the shipped tree mounted, plus `toh-mcp-client`, which it does not mount and which exists so an installed `toh` can. Four remain — the [session-search-not-shipped-default decision](2026-08-02-session-search-not-shipped-default.md) removed `@buckeyestudio/toh-tool-session-query` along with its row.
