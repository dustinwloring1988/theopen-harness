# Agent Note: Omit runtime invariants from shipped toh config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@buckeyestudio/toh-*/invariant` entry.
...
- Ordinary `toh` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
