...
重复初始化是受支持的行为，并且始终最多保留一个存活的由服务器挂载的适配器。并发的 `initialize` 调用按到达顺序在同一个已结算的尾部上排队，因此两次调用不会交叉写入字段或挂载。每个排队运行在工作前检查 shutting-down 标志，并以与关闭后 prompt 相同的错误拒绝。存储的 fiber 会记录它所服务的 provider：当请求的 provider 没有已注册适配器时，运行会先 dispose 已存储的 fiber 再挂载替换者；当重复运行选择的 provider 恰好是自身回退 fiber 注册的那个时，运行会在适配器存在性检查之前释放该回退，使过时的挂载不会被误读为外部负责方；当运行选择了另一个已注册的 provider 时，即使没有替换挂载，也会 dispose 已存储的回退 fiber。dispose 通过同一个辅助方法完成，它在 await 之前先登记这次尝试：成功则释放属主关系，失败则保留已存储的 fiber，让 `shutdown` 可以重试并上报，同时拒绝仍会到达该 initialize 调用方。运行会在每个被等待的清理或挂载步骤之后通过一个辅助方法重读该标志：挂载中途开始的 shutdown 会发现新 fiber 已经存储，其 dispose 因此经由被登记的辅助方法完成，失败的尝试会把 fiber 保留给拆除清理而拒绝仍到达调用方；而其挂载前 dispose 与 shutdown 重叠的运行即使没有替换跟随也会拒绝。`performShutdown` 会在获取拆除清单之前，等待入口处捕获的初始化尾部以及任何已登记的 dispose 尝试，因此其完成会跟随所有已接受的握手，而不是与某次握手赛跑。
...
包测试固定了该生命周期：两次顺序 initialize 会 dispose 第一个挂载，并为 shutdown 恰好留下一个 fiber；重叠的 initialize 在第一个挂载之后串行；与 shutdown 重叠的 initialize 会拒绝并 dispose 自己的挂载；shutdown 在已接受握手的挂载或被替换 dispose 未结算时保持未完成；失败的被替换 dispose 和失败的关闭重叠挂载 dispose 各自保留属主关系，由 shutdown 重试并上报；切换到另一个已注册 provider 时会释放过时的回退适配器而不挂载替换者，且当该 dispose 与 shutdown 重叠时会拒绝；针对自身回退所注册 provider 的重复 initialize 会替换该回退，且当其替换 dispose 与 shutdown 重叠时会拒绝；shutdown 之后的 initialize 不再挂载即被拒绝；真实 harness 运行显示重复 initialize 加 shutdown 之后上下文中不再有 DeepSeek provider。
