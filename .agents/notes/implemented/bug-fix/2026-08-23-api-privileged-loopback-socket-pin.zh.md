...
在全接口组合上，即使 authority 已声明，LAN 调用者也恰好失去特权集合；浏览器不受影响——它们的回环声明此前为真，而来自回环本身的请求依旧不对 socket 复核。本地反向代理仍会呈现回环对端：pin 证明的是连接源自宿主机，而非背后是谁，认证仍是记录在案的延期工作。不经传输层到达 fetch 层的进程内调用者报不出对端地址，在特权面上按失败关闭处理。
