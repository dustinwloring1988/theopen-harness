...
`AclSandboxSpawnOptions` gains `maxOutputBytes`: the per-stream budget, applied independently to stdout and stderr to match the subprocess seam's per-stream collectors. Its default is 64_000 (`DEFAULT_MAX_OUTPUT_BYTES`), the repository's standard per-stream output budget (the `toh-bash-local`/`toh-pwsh-local` `maxOutputBytes` config). `spawn()` resolves the default as an explicit step and rejects a non-positive or non-finite value before spawning — a NaN or Infinity budget would silently disable the trim, so misconfiguration fails loud at the boundary. `stdio: 'inherit'` ignores the option: bytes flow straight through and nothing is buffered.

## Verification

`tests/drain-bound.spec.ts` pins the drain-level tail-keep (sub-cap output intact across polls, both head-drop paths yielding a byte-exact last-`maxBytes` window, a single over-cap chunk trimmed to its tail) and drives REAL confined children on win32 hosts: one over-producing child under a small explicit cap and one under the default budget. The stub harness in `tests/index-failure-paths.spec.ts` pins the spawn-side resolution: an explicit cap applied to both streams, the default applied when the option is omitted, and the fail-loud rejection of invalid values before any spawn.
