...
A wedged runtime costs at most roughly 35 seconds of teardown instead of hanging the caller: shutdown waits are capped at 30 seconds plus the five-second reap join, whatever the stdin write or configuration contains, and waiting for a competing lifecycle transition consumes that same budget instead of extending it. Concurrent `start()` and `close()` callers serialize behind one lifecycle transition, paying single process-launch latency instead of leaking a duplicate runtime or tearing down a half-finished spawn. `close()` is terminal: a closed client cannot be restarted, and callers needing another runtime construct a new client.
