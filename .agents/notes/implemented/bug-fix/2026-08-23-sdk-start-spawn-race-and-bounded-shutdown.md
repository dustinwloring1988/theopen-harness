...
`start()` performs its whole check-and-spawn under the instance lock and assigns `self._proc` inside that locked region, so one client owns at most one runtime subprocess. Lifecycle transitions additionally serialize behind a dedicated lifecycle lock held across spawn in `start()` and across process ownership teardown in `close()`; the instance lock keeps guarding only short shared-state access, so `request()` never contends with teardown, a concurrent `close()` cannot observe an in-progress spawn as an absent process, and no runtime can be stored after `close()` returned.
