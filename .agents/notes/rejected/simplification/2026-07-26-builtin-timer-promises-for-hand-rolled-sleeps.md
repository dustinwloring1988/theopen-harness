...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`toh-llm-mock-server` `pause()`, `toh-lsp-stdio`, `toh-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
