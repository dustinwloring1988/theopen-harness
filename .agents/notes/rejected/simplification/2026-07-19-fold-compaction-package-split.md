...
Compaction is split between `@buckeyestudio/toh-compaction`, which owns an abstract two-method service and shared types, and `@buckeyestudio/toh-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@buckeyestudio/toh-compaction` and remove `@buckeyestudio/toh-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@buckeyestudio/toh-compaction-basic` and its workspace/package metadata are removed.
- `@buckeyestudio/toh-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
